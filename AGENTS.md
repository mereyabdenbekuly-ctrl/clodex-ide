...
- `pnpm -F <package-name> test` - Run tests for specific package (e.g., `pnpm -F @clodex/karton test`)

### Browser App Specific
- `pnpm -F clodex start` - Start the Electron browser app (with typecheck)
- `pnpm -F clodex storybook` - Start Storybook for browser UI components
- `pnpm -F clodex typecheck` - Typecheck browser app only
...
Clodex is a monorepo using pnpm workspaces and Turborepo. The product is a browser-based development tool that connects frontend UI to AI agents in code editors, allowing developers to select DOM elements and have AI agents implement changes.

### Directory Structure

```
apps/
  browser/       - Electron desktop app (main product "Clodex Agentic IDE")
  cli/           - CLI tool for running Clodex Agentic IDE
