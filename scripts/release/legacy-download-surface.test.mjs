...
const communityVersion = '1.16.0-communityobserved21';
const communityTag = `v${communityVersion}`;
const communitySourceCommit = 'd2dd2b63077c67255d60b8ab53f5c3c0995c7f84';
const communityRunId = '30483341383';
...
const installerFileNames = [
  ...
  'clodex-community-observed-1.16.0.communityobserved21-1.x86_64.rpm',
];
...
test('website exposes only the exact verified Community Observed release assets', () => {
  ...
  for (const relativePath of [
    ...
    assert.match(source, /Download Community Observed 21/u);
...
test('post-release documentation stays coherent with the exact observed build', () => {
  ...
  assert.match(landingCopy, /Community Observed 21/gu);
  assert.doesNotMatch(
    landingCopy,
  ...
  assert.match(homePage, /Community Observed 21/u);

  const downloadLayout = readFileSync(
    path.join(websiteDirectory, 'src', 'app', 'download', 'layout.tsx'),
    'utf8',
  );
  assert.match(downloadLayout, /Download CLODEx Community Observed 21/u);
