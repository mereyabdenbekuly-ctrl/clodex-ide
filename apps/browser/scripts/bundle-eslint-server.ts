...
const TEMP_DIR = path.join(os.tmpdir(), 'clodex-eslint-build');
...
async function main() {
  ...
  // The source archive's root postinstall points at generated build helpers
  // that are not included in the GitHub release zip. They are extension
  // packaging helpers, not dependencies of the server webpack build.
