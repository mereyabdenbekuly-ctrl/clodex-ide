  ...
  get registerServerProcedureHandler(): KartonServer<KartonContract>['registerServerProcedureHandler'] {
    const register = (name: string, handler: unknown) => {
      ...
      return this.kartonServer.registerServerProcedureHandler(
        name as never,
        (async (callerId: string, ...args: unknown[]) => {
          const startedAt = performance.now();
          try {
            const result = await callable(callerId, ...args);
            this.procedureEventSink?.({
              name,
              callerId,
    ...
    return register as KartonServer<KartonContract>['registerServerProcedureHandler'];
  }
