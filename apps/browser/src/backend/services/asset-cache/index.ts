...
import { migrateDatabase } from '@clodex/agent-core/migrate-database';
...
const API_URL =
  ...
  (process.env.CLODEX_API_URL as string | undefined) ??
...
export interface AssetCacheServiceOptions {
  dataProtection: DataProtection;
  readFile: (filePath: string) => Promise<Buffer>;
}

function cacheContext(fileHash: string): string {
  return protectedFileContext.cache('asset-read-url', fileHash);
}

function encodeReadUrl(
  dataProtection: DataProtection,
  readUrl: string,
  fileHash: string,
): string {
  return dataProtection.protectString(readUrl, cacheContext(fileHash));
}

function decodeReadUrl(
  dataProtection: DataProtection,
  readUrl: string,
  fileHash: string,
): string {
  if (!isDataProtectionEnvelopeString(readUrl)) {
    throw new Error(
      '[AssetCacheService] Plaintext cache row found after protected migration',
    );
  }
  return dataProtection.unprotectString(readUrl, cacheContext(fileHash));
}

async function migrateProtectedRows(
  db: LibSQLDatabase<Schema>,
  dbDriver: Client,
  dataProtection: DataProtection,
  logger: Logger,
): Promise<number> {
  await dbDriver.execute('PRAGMA secure_delete=ON');
  const rows = await db.select().from(assetCache);
  let migrated = 0;
  for (const row of rows) {
    if (isDataProtectionEnvelopeString(row.readUrl)) {
      decodeReadUrl(dataProtection, row.readUrl, row.fileHash);
      continue;
    }
    await db
      .update(assetCache)
      .set({
        readUrl: encodeReadUrl(dataProtection, row.readUrl, row.fileHash),
      })
      .where(eq(assetCache.fileHash, row.fileHash));
    migrated++;
  }
  if (migrated > 0) {
    await dbDriver.execute('PRAGMA wal_checkpoint(TRUNCATE)');
    await dbDriver.execute('VACUUM');
    await dbDriver.execute('PRAGMA wal_checkpoint(TRUNCATE)');
    logger.info(
      `[AssetCacheService] Migrated ${migrated} plaintext cache row(s)`,
    );
  }
  return migrated;
}

export async function migrateAssetCacheRowsAtStartup(
  dataProtection: DataProtection,
  logger: Logger,
): Promise<number> {
  ...
  try {
    ...
    return await migrateProtectedRows(db, dbDriver, dataProtection, logger);
  } finally {
    dbDriver.close();
  }
}
...
export class AssetCacheService extends DisposableService {
  ...
  private constructor(
    ...
    logger: Logger,
    options: AssetCacheServiceOptions,
  ) {
    super();
    this.db = db;
  ...
  public static async create(
    ...
    const instance = new AssetCacheService(
      db,
      dbDriver,
      getAccessToken,
      logger,
    ...
    await migrateProtectedRows(db, dbDriver, options.dataProtection, logger);
  ...
  public async resolve(
    ...
    if (cached && cached.expiresAt > minExpiresAt) {
      ...
      return {
        type: 'url',
        url: decodeReadUrl(this.dataProtection, cached.readUrl, hash),
      };
    }
    ...
    try {
      ...
      await this.db
        ...
        .values({
          fileHash: hash,
          readUrl: encodeReadUrl(this.dataProtection, data.readUrl, hash),
          expiresAt,
        })
        .onConflictDoUpdate({
