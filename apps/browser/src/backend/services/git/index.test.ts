...
describe('GitService', () => {
  ...
  it('lists all fetch remotes with normalized web URLs', async () => {
    const { service } = await createGitService({
      'remote -v':
    ...
    await expect(service.getRepositoryRemotes('/repo')).resolves.toEqual([
      {
        remoteName: 'origin',
  ...
  it('creates worktrees under the home worktrees directory', async () => {
    ...
    const expectedWorktreePathPattern = escapeRegExp(
      path.join(mockHomeDir, '.clodex', 'worktrees'),
    );
