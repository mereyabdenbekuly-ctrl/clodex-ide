...
import { sanitizeEnv } from '@clodex/agent-shell';
import type {
  GitActionFailure,
...
function parseRemotes(output: string): GitRepositoryRemoteInfo[] {
  ...
  return output
    ...
    .flatMap((match) => {
      ...
      if (seen.has(remoteName)) return [];
      seen.add(remoteName);
      return [{ remoteName, url: match[2]! }];
    });
}
...
export class GitService extends DisposableService {
  ...
  public async getRepositoryRemotes(
    workspacePath: string,
  ): Promise<GitRepositoryRemoteWebInfo[]> {
    const remotesOutput = await this.runGit(workspacePath, ['remote', '-v']);
    if (!remotesOutput) return [];

    return parseRemotes(remotesOutput).map((remote) => ({
      ...remote,
      webUrl: remoteUrlToWebUrl(remote.url),
    }));
  }
