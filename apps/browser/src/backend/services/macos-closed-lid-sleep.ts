...
const SUDOERS_RULE_PATH = '/etc/sudoers.d/clodex-closed-lid-sleep';
const SUDOERS_TEMP_RULE_PATH = '/tmp/clodex-closed-lid-sleep-sudoers';
...
const CLOSED_LID_SLEEP_PERSISTENCE_WARNING =
  'This setting changes a persistent macOS power setting. If clodex quits unexpectedly, closed-lid sleep may remain disabled until you turn it off again.';
...
async function installPasswordlessPmsetRule(): Promise<void> {
  ...
  await execFileAsync(OSASCRIPT_PATH, [
    ...
    `do shell script ${JSON.stringify(command)} with prompt ${JSON.stringify(
      'clodex wants to install a restricted sudoers rule so it can toggle closed-lid sleep without asking every time.',
...
export class MacOSClosedLidSleepService extends DisposableService {
  ...
  private initialize(): void {
    ...
    this.statusInterval = setInterval(() => {
      ...
      if (!this.ownedByClodex && !this.isChanging) return;
      this.syncState().catch((error) => {
        this.logger.debug(
  ...
  private async preventSleep(): Promise<ClosedLidSleepState> {
    ...
    try {
      ...
      this.ownedByClodex = !wasDisabled;
      await this.syncState();
      this.logger.info(
        `[MacOSClosedLidSleepService] Closed-lid sleep prevention enabled. ownedByClodex=${this.ownedByClodex}`,
      );
    } catch (error) {
  ...
  private publishState(isSleepDisabled: boolean): void {
    const state: ClosedLidSleepState = {
      ...
      isSleepDisabled,
      ownedByClodex: this.ownedByClodex,
