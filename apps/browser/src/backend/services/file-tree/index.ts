...
import { getRipgrepPath } from '@clodex/agent-runtime-node';
import { rankPathFuzzyCandidates } from '@clodex/agent-core/mount-manager';
...
export type AttachmentAgentResolver = (attachmentDir: string) => string | null;
...
export class FileTreeService extends DisposableService {
  ...
  private attachmentAgentResolver: AttachmentAgentResolver | null = null;
  private attachmentContentReader: AttachmentContentReader | null = null;
  ...
  setAttachmentReader(
    ...
    this.attachmentAgentResolver = agentResolver;
    this.attachmentContentReader = reader;
  }
  ...
  async getFilePreview(
    ...
    const mount = parseWorkspaceKey(validated.key);
    if (
      mount?.prefix === 'att' &&
      this.attachmentAgentResolver &&
      this.attachmentContentReader
    ) {
      const agentId = this.attachmentAgentResolver(mount.path);
      if (!agentId) return null;
      const content = await this.attachmentContentReader(
        agentId,
        validated.relativePath,
      );
      const kind = this.classifyBuffer(
        content,
        validated.relativePath,
        mimeType,
      );
      ...
      if (kind === 'text' || kind === 'svg') {
        return {
          ...base,
          text: content.subarray(0, MAX_TEXT_BYTES).toString('utf8'),
          truncated: content.byteLength > MAX_TEXT_BYTES,
        };
      }
      return base;
    }
  ...
  private classifyBuffer(
    ...
    const ext = path.basename(fileName).split('.').pop()?.toLowerCase() ?? '';
    if (TEXT_EXTENSIONS.has(ext)) return 'text';
    return content.subarray(0, 4096).includes(0) ? 'binary' : 'text';
  }
