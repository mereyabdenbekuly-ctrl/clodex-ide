...
} from '../network-policy/controlled-browser';
import {
  canBrowserHandleUrl,
...
export class BrowsingTabController extends EventEmitter<TabControllerEventMap> {
  ...
  private readonly controlledBrowserEgress:
  ...
  constructor(
    ...
    this.controlledBrowserEgress = controlledBrowserEgress;
    ...
    if (this.controlledBrowserEgress) {
      enforceControlledBrowserWebRtcPolicy(this.webContentsView.webContents);
    }
   ...
   * Temporarily hides clodex overlay elements during capture via Karton state.
   *
  ...
  public async captureElementScreenshot(
    ...
    if (
      ...
      this.currentState.url.startsWith('clodex://internal/')
    ) {
      return null;
    }
  ...
  private setupAuthenticationHandler() {
    this.authenticationHandler = new TabAuthenticationHandler(
      ...
      this.controlledBrowserEgress?.proxyAuthenticationHandler,
    );
  }
   ...
   * Skips logging for internal clodex:// URLs.
   */
  private async logNavigationToHistory(url: string): Promise<void> {
    // Skip internal URLs
    if (url.startsWith('clodex://')) {
      this.pendingNavigation = null;
      return;
    }
