 ...
 */
const UI_INTERNAL_TAB_PROTOCOLS = new Set(['clodex:', 'app:']);
...
/**
 * Handles `clodex://open-folder-in-ide/<absPath>?ide=<ide>` URLs.
...
export async function openFolderFirstFileInIde(
  ...
  const withoutScheme = url.replace('clodex://open-folder-in-ide/', '');
  ...
  try {
    ...
    if (files.length > 0) {
      ...
      if (ideUrl.startsWith('clodex://reveal-file/')) {
