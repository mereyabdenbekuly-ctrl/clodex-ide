...
export class WindowLayoutService extends DisposableService {
  ...
  /**
   * Bring the main window forward and select an agent created from a
   * secondary native surface such as Quick Task.
   */
  public async focusAgentFromExternalWindow(
    ...
    await this.handleSetLastOpenAgentId(agentInstanceId);

    const window = this.getBaseWindow();
    if (!window) return;
  ...
  private setupBrowserSessionStealth() {
    ...
    // Build the Sec-CH-UA brand list (matches real Chrome format + Clodex)
  ...
  private handleCreateTab = async (
    ...
    try {
      ...
      if (targetUrl?.startsWith('clodex://')) {
  ...
  private handleTogglePanelKeyboardFocus = async (
    ...
    if (panel === 'clodex-ui') {
      ...
      // FIX (Win32): Symmetric with the clodex-ui path above.
  ...
  private handleDeactivateSearchBar = () => {
    ...
    // via togglePanelKeyboardFocus('clodex-ui'), and nothing restores it on
