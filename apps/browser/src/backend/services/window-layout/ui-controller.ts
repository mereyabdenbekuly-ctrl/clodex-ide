...
const UI_SESSION_PARTITION = 'persist:clodex-ui';
...
export interface UIControllerEventMap {
  ...
  togglePanelKeyboardFocus: [panel: 'clodex-ui' | 'tab-content'];
  stop: [tabId?: string];
  reload: [tabId?: string];
...
export class UIController extends EventEmitter<UIControllerEventMap> {
  ...
  private registerAttachmentProtocol(): void {
    ...
    uiSession.protocol.handle('attachment', async (request) => {
      try {
        ...
        const content = await this.attachments.read(agentId, attachmentId);
  ...
  private createView(): WebContentsView {
    ...
    // Intercept non-http navigations in the UI frame (e.g. clodex://reveal-file/)
    view.webContents.on('will-navigate', (event, url) => {
      if (url.startsWith('clodex://reveal-file/')) {
        event.preventDefault();
        const filePath = decodeURIComponent(
          url.replace('clodex://reveal-file/', '').replace(/:\d+$/, ''),
        );
        this.logger.debug(
      ...
      if (url.startsWith('clodex://open-folder-in-ide/')) {
        event.preventDefault();
    ...
    view.webContents.setWindowOpenHandler((details) => {
      ...
      if (details.url.startsWith('clodex://reveal-file/')) {
        const filePath = decodeURIComponent(
          details.url.replace('clodex://reveal-file/', '').replace(/:\d+$/, ''),
        );
        this.logger.debug(
          `[UIController] Revealing file in folder: ${filePath}`,
        );
      ...
      // Intercept clodex://open-folder-in-ide/ — find first file in
      // the directory and open it in the IDE, or reveal in Finder/Explorer.
      if (details.url.startsWith('clodex://open-folder-in-ide/')) {
  ...
  private registerKartonProcedures() {
    ...
    this.uiKarton.registerServerProcedureHandler(
      'appScreen.openProjects',
      async (_callingClientId: string) => {
        this.uiKarton.setState((draft) => {
          draft.appScreen.mode = 'projects';
        });
      },
    );
    this.uiKarton.registerServerProcedureHandler(
      'appScreen.closeProjects',
      async (_callingClientId: string) => {
        this.uiKarton.setState((draft) => {
          draft.appScreen.mode = 'main';
        });
    ...
    this.uiKarton.registerServerProcedureHandler(
      ...
      async (_callingClientId: string, panel: 'clodex-ui' | 'tab-content') => {
  ...
  public forwardFocusEvent(tabId: string) {
    this.view.webContents.send('clodex-tab-focused', tabId);
  }
  ...
  public unregisterKartonProcedures() {
    ...
    this.uiKarton.removeServerProcedureHandler('appScreen.openProjects');
    this.uiKarton.removeServerProcedureHandler('appScreen.closeProjects');
