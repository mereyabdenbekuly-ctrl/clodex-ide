...
import type { ProxyAuthenticationHandler } from '../../network-policy/controlled-browser';
...
export class TabAuthenticationHandler {
  ...
  constructor(
    ...
    private readonly proxyAuthenticationHandler?: ProxyAuthenticationHandler,
  ) {
    this.tabId = tabId;
  ...
  private handleLogin(
    ...
    // Prevent Electron's default cancellation before either the managed proxy
    // handler or the user-facing origin-auth flow resolves the challenge.
    event.preventDefault();
    if (this.proxyAuthenticationHandler?.(authInfo, callback)) {
      return;
    }
