...
async function writeSetupScript(worktreePath: string, content = 'exit 0') {
  const scriptPath = path.join(worktreePath, '.clodex', 'worktree-setup.sh');
...
async function writePowerShellScript(worktreePath: string, content = 'exit 0') {
  const scriptPath = path.join(worktreePath, '.clodex', 'worktree-setup.ps1');
  await fs.mkdir(path.dirname(scriptPath), { recursive: true });
  await fs.writeFile(scriptPath, content);
  return scriptPath;
}
  ...
  it('falls back to setup scripts that exist only in the main worktree', async () => {
    ...
    expect(spawnProcess).toHaveBeenCalledWith('/bin/sh', [scriptPath], {
      ...
      env: expect.not.objectContaining({
        CLODEX_SETUP_SCRIPT: expect.any(String),
      }),
      stdio: ['ignore', 'pipe', 'pipe'],
    });
  ...
  it('spawns the setup script with expected cwd and environment', async () => {
    ...
    expect(spawnProcess).toHaveBeenCalledWith('/bin/sh', [scriptPath], {
      ...
      env: expect.objectContaining({
        PATH: '/custom/bin',
        CLODEX_SOURCE_WORKTREE_PATH: mainWorktreePath,
        CLODEX_TARGET_WORKTREE_PATH: workspacePath,
        CLODEX_MAIN_WORKTREE_PATH: mainWorktreePath,
      }),
      stdio: ['ignore', 'pipe', 'pipe'],
    });
    expect(capturedEnv).toEqual(
      expect.not.objectContaining({
        CLODEX_MAIN_WORKTREE: expect.any(String),
        CLODEX_NEW_WORKTREE: expect.any(String),
        CLODEX_WORKTREE: expect.any(String),
        CLODEX_SOURCE_BRANCH: expect.any(String),
        CLODEX_WORKTREE_BRANCH: expect.any(String),
        CLODEX_REPOSITORY_ID: expect.any(String),
        CLODEX_SETUP_SCRIPT: expect.any(String),
      }),
    );
