...
export class WorktreeSetupRunner {
  ...
  public async start(metadata: WorktreeSetupMetadata): Promise<void> {
    ...
    // resolvedEnv are preserved. CLODEX_*_WORKTREE_PATH vars are
    ...
    const env: NodeJS.ProcessEnv = mergeEnv(sanitizedBase, {
      CLODEX_SOURCE_WORKTREE_PATH: metadata.sourceWorktreePath,
      CLODEX_TARGET_WORKTREE_PATH: metadata.workspacePath,
      CLODEX_MAIN_WORKTREE_PATH: metadata.mainWorktreePath,
    });
