...
export const askUserQuestions = (
  ...
  return tool({
    ...
    execute: async (flatParams) => {
      ...
      return await requestStructuredUserQuestions({
        uiKarton,
...
export async function requestStructuredUserQuestions(options: {
  uiKarton: KartonService;
  hostAgentStateMutations: HostAgentStateMutations;
  agentInstanceId: string;
  params: AskUserQuestionsToolInput;
  signal?: AbortSignal;
  onQuestionRequested?: (agentId: string) => void | Promise<void>;
  ...
  // One visible structured dialog per agent. Replacing a previous dialog
  // resolves its deferred promise instead of leaking it.
  ...
  try {
    const result = await new Promise<AskUserQuestionsToolOutput>((resolve) => {
