...
function makeMockModelProviderService(): ModelProviderService {
  return {
    getModelWithOptionsAsync: vi.fn().mockReturnValue({
      model: { id: 'mock-model' },
      providerOptions: {},
      headers: {},
...
describe('classifyShellCommand', () => {
  ...
  it('returns the classifier result from the first model on success', async () => {
    ...
    expect(mps.getModelWithOptionsAsync).toHaveBeenCalledWith(
  ...
  it('falls back to the second model when the first fails', async () => {
    ...
    expect(mps.getModelWithOptionsAsync).toHaveBeenNthCalledWith(
      2,
      'gpt-5.4-nano',
      'agent-1',
      expect.any(Object),
  ...
  it('sends the strict dependency-install approval policy to the classifier', async () => {
    generateObjectMock.mockResolvedValueOnce({
      object: {
        needsApproval: true,
        explanation: 'Installs dependencies and may run scripts.',
      },
    } as any);

    const mps = makeMockModelProviderService();
    const telemetry = makeMockTelemetryService();
    await classifyShellCommand(
      {
        ...baseInput,
        command: 'pnpm install',
        agentExplanation: 'Install missing dependencies',
      },
      mps,
      'agent-1',
      telemetry,
    ...
    expect(systemPrompt).toContain('Installs or updates dependencies');
    expect(systemPrompt).toContain('pnpm install');
    expect(systemPrompt).toContain('needsApproval": true');
  });
});
