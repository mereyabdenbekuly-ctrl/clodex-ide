...
if ((Test-Path variable:global:__ClodexTerminalState) -and $null -ne $Global:__ClodexTerminalState.OriginalPrompt) {
  return
}
if ($ExecutionContext.SessionState.LanguageMode -ne "FullLanguage") {
  return
}
$Global:__ClodexTerminalState = @{
  OriginalPrompt = $function:Prompt
}
function Global:__ClodexTerminalFileUri([string]$Path) {
  return ([System.Uri]::new($Path)).AbsoluteUri
}
function Global:__ClodexTerminalEmitMetadata() {
  try {
    $Cwd = (Get-Location).ProviderPath
    [Console]::Write("$([char]0x1b)]7;$(__ClodexTerminalFileUri $Cwd)\`a")
  } catch {}
}
function Global:Prompt() {
  Set-StrictMode -Off
  $PromptResult = $Global:__ClodexTerminalState.OriginalPrompt.Invoke()
  __ClodexTerminalEmitMetadata
  return $PromptResult
}
__ClodexTerminalEmitMetadata
`.trim();
...
export class TerminalService extends DisposableService {
  ...
  /**
   * Host-service entry point for creating a user-visible terminal without
   * routing through renderer IPC.
   */
  ...
  /**
   * Host-service entry point for writing trusted handoff input to a terminal.
   * The SSH command is built in the main process and never contains secrets.
   */
  public writeTerminalInput(terminalId: string, data: string): void {
  ...
  constructor(
    ...
    // app config keys/URLs) that would otherwise leak from the clodex
    ...
    // keep normal shell history and don't carry the CLODEX_SHELL marker.
