...
describe('WorktreeSetupSettingsService', () => {
  ...
  it('saves setup scripts only under the repository setup script path', async () => {
    ...
    await expect(
      fs.readFile(path.join(repoPath, '.clodex', 'worktree-setup.sh'), 'utf8'),
  ...
  it('saves the PowerShell variant under the .ps1 path', async () => {
    ...
    expect(result.ok).toBe(true);
    await expect(
      fs.readFile(path.join(repoPath, '.clodex', 'worktree-setup.ps1'), 'utf8'),
    ...
    const readFileSpy = vi
      ...
      .mockImplementation(
        async (
          ...
          if (
            String(filePath) ===
            path.join(unreadableRepo, '.clodex', 'worktree-setup.sh')
          ) {
  ...
  it('rejects deleting paths outside the managed worktree directory', async () => {
    ...
    await expect(
      ...
      ok: false,
      message: 'Worktree is not clodex-managed.',
    });
    expect(gitService.removeWorktree).not.toHaveBeenCalled();
  });
