import type { CommandRegistry, TelemetrySink } from '@clodex/agent-core';
...
export function registerToolboxAttachHandlers(
  ...
  registry.registerCommand<[pendingEditId: string], void>(
    ...
    async (_ctx, [pendingEditId]) => {
      await deps.pendingEdits.acceptEdit(pendingEditId);
      deps.telemetry?.capture('pending-edit-accepted', {
  ...
  registry.registerCommand<[pendingEditId: string, feedback?: string], void>(
    ...
    async (_ctx, [pendingEditId, feedback]) => {
      deps.pendingEdits.rejectEdit(pendingEditId, feedback);
      deps.telemetry?.capture('pending-edit-rejected', {
        pending_edit_id: pendingEditId,
      });
    },
  );
