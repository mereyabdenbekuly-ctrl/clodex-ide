 ...
 * (`toolbox.acceptHunks`, `toolbox.rejectHunks`, pending edit decisions)
 * so the bridge's drift
...
function registerAttachHandlerNoOps(registry: CommandRegistry): void {
  ...
  registry.registerCommand<[pendingEditId: string], void>(
    'toolbox.acceptPendingEdit',
    async () => {},
  );
  registry.registerCommand<[pendingEditId: string, feedback?: string], void>(
    'toolbox.rejectPendingEdit',
    async () => {},
  );
  registry.registerCommand<[agentInstanceId: string], void>(
    'agents.markAsRead',
...
/**
 * Pending edits are the pre-apply diff path used by `write`/`multiEdit`.
 * Swarm workers run in the background, but their tools still write through
 * the shared AgentStore; the bridge must project those proposed edits back
 * into Karton so the active chat footer can render review cards.
 */
describe('AgentCoreBridge (pending proposed edits mirror)', () => {
  function makeDiff(id: string, path = `/w/${id}.ts`): FileDiff {
    return {
      isExternal: false,
      fileId: id,
  ...
  function makePendingEdit(
    id: string,
    overrides: Partial<PendingEditPreview> = {},
  ): PendingEditPreview {
    return {
      id,
      toolCallId: id,
      agentInstanceId: overrides.agentInstanceId ?? 'a1',
      path: overrides.path ?? `/w/${id}.ts`,
      relativePath: overrides.relativePath ?? `w/${id}.ts`,
      status: overrides.status ?? 'pending',
      createdAt: overrides.createdAt ?? 1,
      fileDiff: overrides.fileDiff ?? makeDiff(id, overrides.path),
    };
  }

  function setPendingProposedEdits(
    store: AgentStore,
    agentInstanceId: string,
    ...
    store.update((draft) => {
      ...
      entry.pendingProposedEdits = pendingProposedEdits;
    });
  }

  function createPendingEditsMirrorHarness(initialState?: MockKartonState) {
    const mock = createKartonMock(initialState);
    const store = new AgentStore(createInitialAgentSystemState());
    const activeApp = createActiveAppStateController(store);
    const registry = new CommandRegistry();
  ...
  it('mirrors background worker pending edits into Karton for the same agent', () => {
    const { store, getKartonState } = createPendingEditsMirrorHarness();
    const edit = makePendingEdit('swarm-tool-call-1', {
      path: '/workspace/src/app.ts',
      relativePath: 'wtest/src/app.ts',
    });

    setPendingProposedEdits(store, 'a1', [edit]);

    expect(getKartonState().toolbox.a1?.pendingProposedEdits).toEqual([edit]);
  });

  it('clears mirrored pending proposed edits when the approval resolves', () => {
    const { store, getKartonState } = createPendingEditsMirrorHarness();

    setPendingProposedEdits(store, 'a1', [makePendingEdit('swarm-edit')]);
    setPendingProposedEdits(store, 'a1', []);

    expect(getKartonState().toolbox.a1?.pendingProposedEdits).toEqual([]);
  });

  it('does not rewrite Karton when pending proposed edits keep the same reference', () => {
    const { store, setState } = createPendingEditsMirrorHarness();
    const proposed = [makePendingEdit('same-ref')];

    setPendingProposedEdits(store, 'a1', proposed);
    const callsAfterFirst = setState.mock.calls.length;
    setPendingProposedEdits(store, 'a1', proposed);
