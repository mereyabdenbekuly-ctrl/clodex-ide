import type { HostModels, ModelWithOptions } from '@clodex/agent-core';
import type { ModelCapabilities } from '@clodex/agent-core/types';
 ...
 * `ModelProviderService.getModelWithOptionsAsync`. `metadata` is forwarded
...
export function createBrowserHostModels(
  ...
  async function getWithOptions(
    ...
    try {
      const result = await modelProviderService.getModelWithOptionsAsync(
        modelId as ModelId,
        traceId,
        metadata,
      );
  ...
  return {
    getWithOptions,
    selectModelForTask(request) {
      return modelProviderService.selectModelForTask(request);
    },
...
export function createLazyBrowserHostModels(): LazyBrowserHostModels {
  ...
  const hostModels: HostModels = {
    ...
    selectModelForTask(request) {
      if (!inner) return request.currentModelId;
      return inner.selectModelForTask?.(request) ?? request.currentModelId;
    },
    has(modelId) {
