...
type PendingProposedEditsValue = ToolboxEntry['pendingProposedEdits'];
 ...
 * `CommandRegistry` in `@clodex/agent-core` and mirrors AgentStore
...
export class AgentCoreBridge {
  ...
  private mirrorToKarton(
    ...
    const changes: Array<{
      ...
      pendingProposedEdits?: PendingProposedEditsValue;
    ...
    for (const agentId of agentIds) {
      ...
      const prevProposed = prevEntry?.pendingProposedEdits;
      const nextProposed = nextEntry?.pendingProposedEdits;
      ...
      const proposedChanged =
        prevProposed !== nextProposed &&
        !(prevProposed === undefined && nextProposed?.length === 0) &&
        !(nextProposed === undefined && prevProposed?.length === 0);
      const mountsChanged =
      ...
      if (proposedChanged) entry.pendingProposedEdits = nextProposed ?? [];
    ...
    this.karton.setState((draft) => {
      ...
      for (const change of changes) {
        ...
        if ('pendingProposedEdits' in change) {
          kartonEntry.pendingProposedEdits = (change.pendingProposedEdits ??
            []) as typeof kartonEntry.pendingProposedEdits;
        }
