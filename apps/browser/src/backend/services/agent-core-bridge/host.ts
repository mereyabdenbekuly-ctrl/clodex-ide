...
import { SHELLS_DOMAIN_ID } from '@clodex/agent-shell/env';
...
import { DEFAULT_WORKSPACE_MD_RELATIVE_PATH } from '@clodex/agent-core/mount-manager';
...
import { textBlobTransformer } from '@clodex/agent-core/file-read-transformer';
import type { FileTransformer } from '@clodex/agent-core';
import type { AgentPersonality } from '@shared/karton-contracts/ui/shared-types';
import { buildBrowserSoulPrompt } from '@/agents/chat/personality';
import type { CollaborationMode } from '@shared/collaboration-modes';

export interface BrowserAgentHostDeps {
  logger: BrowserLogger;
  telemetryService: TelemetryService;
  /** OS-keychain-backed protector used by agent-core persistence. */
  ...
  /** Initial communication style for interactive CHAT/MAGUS agents. */
  agentPersonality?: AgentPersonality;
  /** Initial workflow preset for interactive CHAT/MAGUS agents. */
...
export function createBrowserAgentHost(deps: BrowserAgentHostDeps): AgentHost {
  ...
  const host = new AgentHost({
    ...
    models,
    logger: deps.logger,
    dataProtection: deps.dataProtection,
  ...
  applyBrowserAgentBehavior(
    host,
    deps.agentPersonality ?? 'pragmatic',
    deps.collaborationMode ?? 'default',
  );

  // WORKSPACE_MD — the thin "edit `.clodex/WORKSPACE.md` only"
...
/**
 * Rebuild the CHAT/MAGUS profiles with the selected communication and workflow
 * settings. Replacing the profile is synchronous, so the next turn observes
 * the new system prompt without restarting existing agent instances.
 */
export function applyBrowserAgentBehavior(
  host: AgentHost,
  personality: AgentPersonality,
  collaborationMode: CollaborationMode,
): void {
  for (const agentType of [AgentTypes.CHAT, AgentTypes.MAGUS]) {
    host.defineAgentProfile(agentType, {
      envDomainIds: BROWSER_CHAT_ENV_DOMAIN_IDS,
      outputProtocols: BROWSER_OUTPUT_PROTOCOLS,
      systemPromptFragments: {
        intro: browserIntroPrompt,
        soul: buildBrowserSoulPrompt(personality, collaborationMode),
 ...
 * provided by `textBlobTransformer` in `@clodex/agent-core`.
 */
 ...
 * core default (`.clodex/WORKSPACE.md`) so existing workspaces keep
 * working unchanged.
 */
