...
import { applyBrowserAgentBehavior, createBrowserAgentHost } from './host';
...
describe('createBrowserHostModels', () => {
  it('delegates get() to getModelWithOptionsAsync and returns the model', async () => {
    ...
    expect(mp.getModelWithOptionsAsync).toHaveBeenCalledWith(
      'claude-x',
      'trace-1',
      undefined,
    );
  ...
  it('getWithOptions returns the full ModelWithOptions payload', async () => {
    ...
    const mp = {
      modelExists: vi.fn(() => true),
      getModelWithOptionsAsync: vi.fn(() => full),
    };
  ...
  it('delegates selectModelForTask() when the browser provider supports routing', async () => {
    const mp = {
      modelExists: vi.fn(),
      getModelWithOptionsAsync: vi.fn(),
      selectModelForTask: vi.fn(() => 'gemini-3.5-flash'),
    };
    const models = createBrowserHostModels(
      mp as unknown as ModelProviderService,
    );
    ...
    ).resolves.toBe('gemini-3.5-flash');
    expect(mp.selectModelForTask).toHaveBeenCalledWith({
      currentModelId: 'claude-opus-4.7',
      taskRole: 'analysis',
      agentType: 'chat',
      traceId: 'trace-1',
...
describe('createBrowserAgentHost', () => {
  ...
  it('passes a caller-supplied data protector through the host seam', () => {
    const dataProtection = {
      isProtectedString: vi.fn(),
      protectString: vi.fn(),
      unprotectString: vi.fn(),
      isProtectedBuffer: vi.fn(),
      protectBuffer: vi.fn(),
      unprotectBuffer: vi.fn(),
    } as unknown as DataProtection;
    const host = createBrowserAgentHost({
      logger: makeLogger() as unknown as BrowserLogger,
      modelProviderService: {} as ModelProviderService,
      telemetryService: {} as TelemetryService,
      dataProtection,
    });

    expect(host.dataProtection).toBe(dataProtection);
  });

  it('updates CHAT and MAGUS prompts when personality changes', () => {
    const host = createBrowserAgentHost({
      logger: makeLogger() as unknown as BrowserLogger,
      modelProviderService: {} as ModelProviderService,
      telemetryService: {} as TelemetryService,
    ...
    expect(
      host.getAgentProfile(AgentTypes.CHAT)?.systemPromptFragments?.soul,
    ).toContain('Active personality: Friendly.');
    expect(
      host.getAgentProfile(AgentTypes.MAGUS)?.systemPromptFragments?.soul,
    ).toContain('Active personality: Friendly.');
    expect(
      host.getAgentProfile(AgentTypes.CHAT)?.systemPromptFragments?.soul,
    ).toContain('Active collaboration mode: Review.');

    applyBrowserAgentBehavior(host, 'pragmatic', 'implement');

    expect(
      host.getAgentProfile(AgentTypes.CHAT)?.systemPromptFragments?.soul,
    ).toContain('Active personality: Pragmatic.');
    expect(
      host.getAgentProfile(AgentTypes.MAGUS)?.systemPromptFragments?.soul,
    ).toContain('Active personality: Pragmatic.');
    expect(
      host.getAgentProfile(AgentTypes.CHAT)?.systemPromptFragments?.soul,
    ).toContain('Active collaboration mode: Implement.');
  });
});
