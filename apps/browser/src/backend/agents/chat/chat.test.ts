...
describe('BrowserChatAgent', () => {
  ...
  it('merges dynamic Clodex MCP tools from the browser toolbox', async () => {
    const getTool = vi.fn().mockResolvedValue({ kind: 'host-tool' });
    const getClodexMcpTools = vi.fn().mockResolvedValue({
      mcp_clodex_tcp_check: { kind: 'mcp-tool' },
    });
    const instance = Object.create(BrowserChatAgent.prototype) as {
      instanceId: string;
      toolbox: {
        getTool: typeof getTool;
        getClodexMcpTools: typeof getClodexMcpTools;
      };
      getAdditionalTools: () => Promise<Record<string, unknown>>;
    };
    instance.instanceId = 'browser-chat';
    instance.toolbox = { getTool, getClodexMcpTools };

    const extra = await instance.getAdditionalTools();

    expect(getClodexMcpTools).toHaveBeenCalledWith('browser-chat');
    expect(extra.mcp_clodex_tcp_check).toEqual({ kind: 'mcp-tool' });
  });
});
