import { defineToolPartSerializers } from '@clodex/agent-core/agents';
...
export const browserToolPartSerializers = defineToolPartSerializers(
  allToolSchemas,
  {
    ...
    addMemory: ({ input, output, err }) =>
      `[memory: save ${esc(output?.memory.title ?? input.title)} (${esc(input.scope ?? 'agent')})${err ?? ''}]`,

    listMemories: ({ input, output, err }) =>
      `[memory: list ${esc(input.scope ?? 'accessible')} → ${output?.memories.length ?? '?'}${err ?? ''}]`,

    readMemory: ({ input, output, err }) =>
      `[memory: read ${esc(output?.memory?.title ?? input.id)}${err ?? ''}]`,

    searchMemories: ({ input, output, err }) =>
      `[memory: search ${esc(input.query)} → ${output?.matches.length ?? '?'}${err ?? ''}]`,

    deleteMemory: ({ input, output, err }) =>
      `[memory: delete ${esc(input.id)} → ${output?.deleted ? 'deleted' : 'not found'}${err ?? ''}]`,
  },
);
