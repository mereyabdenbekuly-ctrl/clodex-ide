 ...
 *   descriptors (`~/.clodex/skills`, `~/.agents/skills`) with a
...
export function createBrowserHostEnvironmentSources(
  ...
  return {
    ...
    getRuntimeContext(agentInstanceId: string): RuntimeContextSnapshot {
      const activeTabId = karton.state.contentTabs?.activeTabId;
      const activeTab = activeTabId
        ? karton.state.contentTabs.tabs[activeTabId]
        : undefined;
      const activeFile =
        activeTab?.type === 'file' && activeTab.file
          ? `${workspaceKeyToMountPrefix(activeTab.file.workspaceKey)}/${activeTab.file.relativePath}`
          : null;
      ...
      return {
        ...
        activeFilePath:
          ...
          activeTab.agentInstanceId === agentInstanceId
            ? activeFile
            : null,
      };
    },
...
function workspaceKeyToMountPrefix(workspaceKey: string): string {
  const separatorIndex = workspaceKey.indexOf(':');
  return separatorIndex > 0
    ? workspaceKey.slice(0, separatorIndex)
    : workspaceKey;
}
