...
export function getThinkingProviderForRoute({
  ...
  // The Clodex AI SDK provider only reads options from the `clodex`
  // namespace. Provider-native namespaces are relevant only when the IDE
  // connects directly to that provider.
  if (providerMode === 'clodex') return 'clodex';
...
export function getDefaultThinkingSelection(
  ...
  const provider = options[0]?.provider ?? 'clodex';
  const providerDefault = getProviderDefaultValue(
    model.providerOptions,
    provider,
  );
  const clodexDefault = getProviderDefaultValue(
    model.providerOptions,
    'clodex',
  );
  const preferredValue =
    provider === 'clodex'
      ? (clodexDefault ?? providerDefault)
      : (providerDefault ?? clodexDefault);
...
function isMatchingOverrideProvider(
  ...
  return (
    ...
    (route?.providerMode === 'clodex' && overrideProvider === 'clodex')
  );
}
...
function getProviderDefaultValue(
  ...
  switch (provider) {
    case 'clodex': {
      ...
      if (!isPlainObject(clodex)) return undefined;
