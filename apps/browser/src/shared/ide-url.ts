...
/**
 * Generates a `clodex://open-folder-in-ide/` URL. The backend handler
...
export const getFolderIDEUrl = (
  ...
  const clean = absFolderPath.endsWith('/')
    ...
    : absFolderPath;
  return `clodex://open-folder-in-ide/${clean}?ide=${ide}`;
};
...
export const getIDEFileUrl = (
  ...
  switch (ide) {
    ...
    case 'kiro':
      ...
      break;
    case 'other':
      url = `clodex://reveal-file/${encodeURIComponent(absFilePath)}`;
      break;
  }
