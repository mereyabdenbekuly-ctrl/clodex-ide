 ...
 * - `posix`      -> `.clodex/worktree-setup.sh` (executed via `/bin/sh`)
 * - `powershell` -> `.clodex/worktree-setup.ps1` (executed via PowerShell)
 *
...
export const WORKTREE_SETUP_SCRIPT_RELATIVE_PATHS: Record<
  ...
  string
> = {
  posix: '.clodex/worktree-setup.sh',
  powershell: '.clodex/worktree-setup.ps1',
};
