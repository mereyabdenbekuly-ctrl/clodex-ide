...
ipcRenderer.on('clodex-tab-focused', (_event, tabId) => {
  window.dispatchEvent(
    new CustomEvent('clodex-tab-focused', { detail: tabId }),
  );
});
...
contextBridge.exposeInMainWorld('electron', {
  ...
  quickTaskWindow: {
    open: async (initialPrompt?: string): Promise<boolean> => {
      return ipcRenderer.invoke(QUICK_TASK_WINDOW_CHANNELS.show, {
        initialPrompt: initialPrompt ?? '',
        toggle: false,
      });
    },
    toggle: async (initialPrompt?: string): Promise<boolean> => {
      return ipcRenderer.invoke(QUICK_TASK_WINDOW_CHANNELS.show, {
        initialPrompt: initialPrompt ?? '',
        toggle: true,
      });
