  ...
  useLayoutEffect(() => {
    ...
    const checkHoverState = () => {
      ...
      if (elementAtPoint) {
        ...
        const isQuickTaskModalActive =
          document.querySelector('[data-quick-task-modal-active]') !== null;

        if (
          !isElementSelectorOverlay &&
          !isOmniboxModalActive &&
          !isCommandCenterModalActive &&
          !isQuickTaskModalActive
        ) {
      ...
      if (lastInteractive !== isHovering) {
        void movePanelToForeground(isHovering ? 'tab-content' : 'clodex-ui');
