...
declare global {
  interface Window {
    __clodex_shiki_worker_proxy?: ShikiWorkerProxy;
  }
}
...
export function getShikiWorkerProxy(): ShikiWorkerProxy {
  if (!window.__clodex_shiki_worker_proxy) {
    window.__clodex_shiki_worker_proxy = new ShikiWorkerProxy();
  }
  return window.__clodex_shiki_worker_proxy;
}
