...
export class HighlightCache {
  ...
  async hydrateFromIDB(): Promise<void> {
    ...
    try {
      const oldKey = 'clodex_highlight_cache';
...
declare global {
  interface Window {
    __clodex_highlight_cache?: HighlightCache;
  }
}
...
export function getHighlightCache(): HighlightCache {
  if (!window.__clodex_highlight_cache) {
    window.__clodex_highlight_cache = new HighlightCache();

    // Proactively hydrate cache during idle time instead of
    // lazily on first code block render.
    const cache = window.__clodex_highlight_cache;
    if (typeof requestIdleCallback === 'function')
      requestIdleCallback(() => cache.warmup());
    else setTimeout(() => cache.warmup(), 200);
  }

  return window.__clodex_highlight_cache;
}
