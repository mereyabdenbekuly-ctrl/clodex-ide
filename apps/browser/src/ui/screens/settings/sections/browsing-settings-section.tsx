    ...
    <div className="space-y-3">
      ...
      <RadioGroup value={pageSetting.type} onValueChange={handleTypeChange}>
        <RadioLabel>
          <Radio value="home" />
          <div className="flex flex-col">
            <span className="font-medium text-foreground">Clodex Home</span>
            <span className="text-muted-foreground text-xs">
              Open the clodex home page
            </span>
          </div>
        </RadioLabel>
...
function StartupPageSetting() {
  return (
    <PageSettingComponent
      ...
      description="Choose what page opens when clodex starts."
    />
  );
}
...
export function BrowsingSettingsSection() {
  ...
  const defaultSearchEngine =
    searchEngines.find(
      (engine) => engine.id === preferences.search.defaultEngineId,
    )?.shortName ?? 'Not set';
  const overrideHosts = new Set<string>();
  for (const permissionType of configurablePermissionTypes) {
    for (const host of Object.keys(
      preferences.permissions?.exceptions?.[permissionType] ?? {},
    )) {
      overrideHosts.add(host);
    }
  }
  ...
  return (
    <SettingsPage
      eyebrow="Browser"
      title="Browsing"
      description="Configure search, startup pages, and the default permission boundary for websites."
      toolbar={
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          ...
          <SettingsSummaryCard
            ...
            value={searchEngines.length}
            icon={<Globe2Icon className="size-4" />}
          />
          <SettingsSummaryCard
            label="sites with overrides"
            value={overrideHosts.size}
      ...
      <div className="space-y-8">
        <section className="space-y-3">
          <SettingsSectionHeader
            ...
            description="Choose how searches and new browser sessions begin."
          />
        ...
        <section className="space-y-3">
          <SettingsSectionHeader
            ...
            description="Manage global defaults and site-specific permission overrides."
          />
          <SettingsPanel className="divide-y divide-token-border-light overflow-hidden">
            <div className="p-4 sm:p-5">
