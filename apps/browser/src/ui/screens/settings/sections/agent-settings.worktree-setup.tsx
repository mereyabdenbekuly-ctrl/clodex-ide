...
import { Tabs, TabsList, TabsTrigger } from '@clodex/stage-ui/components/tabs';
...
# Runs after clodex creates and mounts a new Git worktree.
...
export function WorktreeSetupSection() {
  ...
  const managedWorktreeCount = repositories.reduce(
    (total, repository) => total + repository.managedWorktrees.length,
    0,
  );
  const configuredScriptCount = repositories.reduce(
    (total, repository) =>
      total +
      Object.values(repository.scripts).filter((script) => script.exists)
        .length,
    0,
  );
  ...
  return (
    <SettingsPage
      ...
      title="Worktrees"
      description="Configure setup scripts and clean Clodex-managed Git worktrees."
      toolbar={
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          ...
          <SettingsSummaryCard
            ...
            value={managedWorktreeCount}
            icon={<GitBranchIcon className="size-4" />}
          />
          <SettingsSummaryCard
            label="setup scripts"
            value={configuredScriptCount}
    ...
    >
      <div className="space-y-8">
        <section className="space-y-3">
          <SettingsSectionHeader
            title="Repositories"
            description="Choose a repository to edit its worktree setup and managed checkouts."
          />
          <RepositoryList
        ...
        <section className="min-w-0">
          {selectedRepository ? (
            <SettingsPanel className="p-4 sm:p-5">
              <RepositoryDetails
            ...
            <SettingsPanel className="flex min-h-80 items-center justify-center p-6 text-center">
              <p className="max-w-md text-sm text-token-text-secondary leading-6">
                {loading
      ...
      <PopoverContent side="top" align="start" className="w-80 gap-4 p-3">
        ...
        <div className="space-y-3">
          <SetupVariableItem
            ...
            value="CLODEX_SOURCE_WORKTREE_PATH"
          />
          <SetupVariableItem
            label="Target worktree path"
            value="CLODEX_TARGET_WORKTREE_PATH"
          />
          <SetupVariableItem
            label="Main worktree path"
            value="CLODEX_MAIN_WORKTREE_PATH"
          />
        </div>
        <PopoverFooter>
          <p className="text-muted-foreground text-xs">
            In POSIX shell scripts, access these via{' '}
            <code className="font-mono">$CLODEX_...</code>. In PowerShell, use{' '}
            <code className="font-mono">$env:CLODEX_...</code>.
          </p>
        </PopoverFooter>
      </PopoverContent>
    </Popover>
  ...
  return (
    <section className="space-y-3">
      <div>
        ...
        <p className="text-muted-foreground text-xs">
          Clodex-controlled worktree instances for this repository.
        </p>
      </div>
      ...
      {worktrees.length === 0 ? (
        <div className="rounded-lg border border-derived-subtle p-4">
          <p className="text-center text-muted-foreground text-sm">
            No clodex-managed worktrees for this repository.
          </p>
        </div>
      ) : (
        <OverlayScrollbar
