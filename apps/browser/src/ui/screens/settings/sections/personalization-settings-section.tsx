...
const AGENT_PERSONALITY_ITEMS: Array<{
...
function AgentPersonalitySetting() {
  const preferences = useKartonState((s) => s.preferences);
  const updatePreferences = useKartonProcedure((p) => p.preferences.update);
  const personality = preferences.agent.personality;

  const handlePersonalityChange = async (value: unknown) => {
    if (!isAgentPersonality(value) || value === personality) {
      return;
    }

    const [, patches] = produceWithPatches(preferences, (draft) => {
      draft.agent.personality = value;
    });
    ...
    <div className="flex items-center justify-between gap-4">
      <div className="min-w-0">
        ...
        <p className="mt-1 text-token-text-secondary text-xs leading-5">
          Choose how coding agents communicate. This does not change their
      ...
      <Select
        ...
        items={AGENT_PERSONALITY_ITEMS}
        triggerVariant="secondary"
        size="xs"
        triggerClassName="w-auto min-w-32 rounded-lg px-2 py-3"
        side="bottom"
        align="end"
      />
    </div>
...
function MascotOverlaySetting() {
  const preferences = useKartonState((s) => s.preferences);
  const updatePreferences = useKartonProcedure((p) => p.preferences.update);
  const persistedSize = preferences.mascotOverlay.size;
  const [localSize, setLocalSize] = useState(persistedSize);

  useEffect(() => {
    setLocalSize(persistedSize);
  }, [persistedSize]);

  const handleSizeCommitted = async (value: number) => {
    const nextSize = Math.max(
      MASCOT_OVERLAY_MIN_SIZE,
      Math.min(MASCOT_OVERLAY_MAX_SIZE, Math.round(value)),
    );

    if (nextSize === persistedSize) {
      setLocalSize(persistedSize);
      return;
    }

    const [, patches] = produceWithPatches(preferences, (draft) => {
      draft.mascotOverlay.size = nextSize;
    });
    ...
    try {
      ...
      setLocalSize(persistedSize);
      console.error('Failed to save mascot size', error);
    }
  };

  const handleResetPosition = async () => {
    if (preferences.mascotOverlay.position === null) return;
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-4">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-token-text-secondary text-xs leading-5">
            Choose a size between {MASCOT_OVERLAY_MIN_SIZE}px and{' '}
            {MASCOT_OVERLAY_MAX_SIZE}px.
          </p>
        </div>
        ...
        <div className="w-40 space-y-1">
          <Slider
            ...
            step={8}
            ariaLabel="Mascot size"
            onValueChange={setLocalSize}
            onValueCommitted={handleSizeCommitted}
          />
          <div className="text-right text-[11px] text-token-text-tertiary tabular-nums">
            {localSize}px
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-token-text-secondary text-xs leading-5">
            Return the mascot to its default bottom-right position.
          </p>
        </div>
        ...
        <Button
          ...
          disabled={preferences.mascotOverlay.position === null}
...
function UiSizeSetting() {
  ...
  return (
    <div className="flex items-center justify-between gap-4">
      <div className="min-w-0">
        <h3 className="font-medium text-sm text-token-text-primary">UI size</h3>
        <p className="mt-1 text-token-text-secondary text-xs leading-5">
          Scale the Clodex interface independently from web page zoom.
        </p>
      </div>
      ...
      <div className="w-36 space-y-1">
        ...
        />
        <div className="flex justify-between text-[11px] text-token-text-tertiary">
          <span>Small</span>
          <span>Default</span>
          <span>Large</span>
        </div>
  ...
  return (
    <div className="flex items-center justify-between gap-4">
      <div className="min-w-0">
        ...
        <p className="mt-1 text-token-text-secondary text-xs leading-5">
          Choose whether Clodex follows your system appearance or always uses
          light or dark mode.
        </p>
      </div>
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-token-text-secondary text-xs leading-5">
          Adapt the color style of your Clodex setup to your liking.
        </p>
      </div>
...
export function PersonalizationSettingsSection() {
  ...
  const personality = useKartonState((s) => s.preferences.agent.personality);
    ...
    <SettingsPage
      ...
      toolbar={
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <SettingsSummaryCard
            accent
            label={`${appColorScheme} appearance`}
            value={getPersonalizationTheme(themeId).name}
          ...
          <SettingsSummaryCard
            ...
            value={
              AGENT_PERSONALITY_ITEMS.find((item) => item.value === personality)
                ?.label ?? personality
            }
    ...
    >
      <div className="space-y-8">
        <section className="space-y-3">
          <SettingsSectionHeader
            title="Appearance"
            description="Choose the system mode, color theme, and interface scale."
          />
        ...
        <section className="space-y-3">
          <SettingsSectionHeader
            ...
            description="Adjust the tone used by coding agents without changing permissions."
          />
          <SettingsPanel className="p-4">
            <AgentPersonalitySetting />
          </SettingsPanel>
        </section>

        {mascotOverlayEnabled && (
          <section className="space-y-3">
            <SettingsSectionHeader
              title="Mascot overlay"
              description="Customize the draggable agent-status mascot shown over the main workspace."
            />
            <SettingsPanel className="p-4">
              <MascotOverlaySetting />
            </SettingsPanel>
          </section>
        )}
