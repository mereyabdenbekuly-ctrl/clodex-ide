...
export function CustomProvidersSection() {
  ...
  const customEndpoints = useKartonState(
    (s) => s.preferences.customEndpoints ?? [],
  );
  const protocolCount = new Set(
    customEndpoints.map((endpoint) => endpoint.apiSpec),
  ).size;
  ...
  return (
    <SettingsPage
      ...
      description="Add custom API endpoints for self-hosted or third-party LLM services."
      actions={
        <Button
          variant="secondary"
          size="sm"
    ...
    >
      <section className="space-y-4">
        <SettingsSectionHeader
          ...
          description="Credentials remain in the secure credential store; this page manages endpoint metadata and routing."
        />
