...
export function CommandCenter() {
  ...
  const activeGitWorkspacePath = useKartonState((state) => {
    const agentMounts = openAgent
      ? state.toolbox[openAgent]?.workspace.mounts
      : undefined;
    return (
      agentMounts?.find((mount) => mount.git !== null)?.path ??
      state.workspaceMounts.find((mount) => mount.git !== null)?.path ??
      null
    );
  });
  ...
  useEffect(() => {
    ...
    void movePanelToForegroundRef.current('clodex-ui');
    void togglePanelKeyboardFocusRef.current('clodex-ui');
  }, [isOpen]);
  ...
  const executeItem = useCallback(
    (item: CommandCenterItem) => {
      ...
      } else if (item.kind === 'action' && item.actionId === 'open-projects') {
        void openProjects();
      } else if (
        item.kind === 'action' &&
        item.actionId === 'open-generated-apps'
      ) {
        void createTab('clodex://internal/generated-apps', true, openAgent);
      } else if (
        item.kind === 'action' &&
        item.actionId === 'open-plugin-library'
      ) {
        void createTab('clodex://internal/plugins', true, openAgent);
      } else if (
        item.kind === 'action' &&
        item.actionId === 'open-quick-task'
      ) {
        quickTask.open();
      } else if (
        item.kind === 'action' &&
        item.actionId === 'review-hosted-pr'
      ) {
        const query = activeGitWorkspacePath
          ? `?workspace=${encodeURIComponent(activeGitWorkspacePath)}`
          : '';
        void createTab(
          `clodex://internal/pull-request${query}`,
          true,
          openAgent,
        );
      }
      ...
      >
        <CommandCenterPanel>
          <CommandCenterInput
            ...
            activeDescendantId={
              selectedItem
                ? getCommandCenterItemDomId(selectedItem.id)
                : undefined
            }
