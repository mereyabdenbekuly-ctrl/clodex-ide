...
export function mergeAgentEntries({
  ...
  const historyEntries: MergedAgentEntry[] = historyList
    ...
    .map((e) => ({
      ...
      projectRootPath:
        e.projectRootPath ?? e.mountedWorkspaces?.[0]?.path ?? null,
      ...
      return {
        ...
        mountedWorkspaces:
          a.mountedWorkspaces.length > 0
            ? a.mountedWorkspaces
            : (h?.mountedWorkspaces ?? []),
        projectId: a.projectId ?? h?.projectId,
        projectRootPath:
          a.projectRootPath ??
          h?.projectRootPath ??
          a.mountedWorkspaces[0]?.path ??
          h?.mountedWorkspaces?.[0]?.path ??
          null,
        projectName: a.projectName ?? h?.projectName,
        isLive: true,
        lastMessageAt:
          a.lastMessageAt > 0
            ? a.lastMessageAt
...
function getProjectRootPath(agent: MergedAgentEntry): string | null {
  return agent.projectRootPath ?? agent.mountedWorkspaces[0]?.path ?? null;
}

function getProjectGroupKey(agent: MergedAgentEntry): string {
  const rootPath = getProjectRootPath(agent);
  return rootPath ? `project:${normalizePath(rootPath)}` : 'project:__none__';
}

function getProjectGroupLabel(agent: MergedAgentEntry): string {
  const rootPath = getProjectRootPath(agent);
  return (
    agent.projectName ??
    (rootPath ? getBaseName(rootPath) || rootPath : 'No project')
  );
}
...
export function buildProjectSessionGroups(
  ...
  for (const agent of entries) {
    const key = getProjectGroupKey(agent);
    let group = groups.get(key);
    if (!group) {
      group = {
        key,
        label: getProjectGroupLabel(agent),
        rootPath: getProjectRootPath(agent),
        severity: null,
        updatedAt: Math.max(agent.lastMessageAt, agent.createdAt),
        agents: [],
      };
  ...
  for (const group of Array.from(groups.values())) {
    group.agents = sortAgentEntriesNewestFirst(group.agents);
    group.severity = maxSeverity(
      group.agents.map((agent) => getAgentStateSeverity(agent)),
    );
  }
