...
import { SwarmInlineProgress } from './swarm-inline-progress';
...
function getSwarmRunFingerprint(run: SwarmRunState | null): string {
  if (!run) return '';
  return [
    run.id,
    run.status,
...
export const ChatHistory = () => {
  ...
  const activeSwarmRun = useKartonState((s): SwarmRunState | null => {
    if (!openAgent) return null;
    return (
      Object.values(s.swarmRuns)
        .filter(
          (run) =>
            run.agentInstanceId === openAgent && run.status === 'running',
        )
        .sort((a, b) => b.startedAt - a.startedAt)[0] ?? null
    );
  });
  const diffArtifactFingerprint = useKartonState((s): string => {
    if (!openAgent) return '';
    const toolbox = s.toolbox[openAgent];
    if (!toolbox) return '';
    return [
      ...(toolbox.pendingProposedEdits ?? []).map(
        (edit) => `p:${edit.id}:${edit.fileDiff.fileId}`,
      ),
      ...(toolbox.pendingFileDiffs ?? []).map(
        (diff) =>
          `d:${diff.fileId}:${diff.path}:${diff.baselineOid}:${diff.currentOid}`,
      ),
      ...(toolbox.editSummary ?? []).map(
        (diff) =>
          `s:${diff.fileId}:${diff.path}:${diff.baselineOid}:${diff.currentOid}`,
      ),
    ].join('|');
  });
  ...
  // Clear pending-working bridge once regular agent work or Swarm progress
  // catches up with the optimistic user bubble.
  useEffect(() => {
    if (isWorking || activeSwarmRun) pendingWorkingRef.current = false;
  }, [isWorking, activeSwarmRun]);
  ...
  const activeSwarmRunRef = useRef(activeSwarmRun);
  activeSwarmRunRef.current = activeSwarmRun;
  ...
  const prevActiveSwarmFingerprintRef = useRef(
    getSwarmRunFingerprint(activeSwarmRun),
  );
  const prevDiffArtifactFingerprintRef = useRef(diffArtifactFingerprint);
  ...
  const cacheGen = useMemo(() => {
    ...
    const activeSwarmFingerprint = getSwarmRunFingerprint(activeSwarmRun);
    const activeSwarmChange =
      prevActiveSwarmFingerprintRef.current !== activeSwarmFingerprint;
    const diffArtifactChange =
      prevDiffArtifactFingerprintRef.current !== diffArtifactFingerprint;
    ...
    } else if (isWorkingChange || activeSwarmChange) {
      // All cached user messages need re-rendering because MessageUser
      // receives isWorking, editability can change, and the latest user
      // message hosts inline Swarm progress.
      userMsgIdsInCacheRef.current.forEach((id) => {
        elementCacheRef.current.delete(id);
      });
      userMsgIdsInCacheRef.current.clear();
    } else if (diffArtifactChange) {
      for (const message of filteredMessages) {
        if (message.metadata?.swarmDiffArtifact) {
          elementCacheRef.current.delete(message.id);
        }
      }
  ...
  const itemContent = useCallback(
    (index: number, message: AgentMessage) => {
      ...
      const curActiveSwarmRun = activeSwarmRunRef.current;
        ...
        return (
          ...
          >
            <div className="pl-6" style={{ paddingRight }}>
              ...
              {curActiveSwarmRun && (
                <SwarmInlineProgress run={curActiveSwarmRun} />
              )}
      ...
      <MountedPathsProvider value={resolvedMounts}>
        <AttachmentMetadataProvider messages={filteredMessages}>
          <section
            ...
            className={cn(
              'pointer-events-auto flex h-full min-h-0 flex-1 text-foreground text-sm focus-within:outline-none focus:outline-none',
            )}
