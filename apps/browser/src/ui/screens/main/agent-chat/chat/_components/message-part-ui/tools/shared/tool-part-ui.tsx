    ...
    return (
      <div
        className={cn(
          ...
          props.showBorder &&
            'codex-tool-card min-h-8 rounded-xl border px-3 shadow-none',
        )}
      >
        {props.trigger}
      </div>
  ...
  return (
    <div
      className={cn(
        ...
        showBorder && 'codex-tool-card rounded-xl border',
      )}
    >
      <Collapsible open={expanded} onOpenChange={setExpanded}>
        <CollapsibleTrigger
          ...
          className={cn(
            ...
            'cursor-pointer',
            showBorder &&
              'min-h-8 rounded-t-xl rounded-b-none border-b bg-transparent px-3',
            // Always have border-b, toggle color to avoid transition-all animating border
            showBorder &&
              (expanded ? 'border-token-border-light' : 'border-transparent'),
            !showBorder &&
          ...
          {contentFooter && (
            <div
              className={cn(
                ...
                'flex select-none flex-row items-center justify-start gap-1 rounded-b-xl bg-token-bg-secondary/45 px-2 py-1 text-token-text-tertiary',
                // Default (absolute) mode: pin to bottom and add separator
                !contentFooterStatic &&
                  'absolute right-0 bottom-0 left-0 h-7 border-token-border-light border-t',
