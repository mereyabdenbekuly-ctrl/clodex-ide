...
export interface StatusCardSection {
  ...
  defaultOpen?: boolean;
  /**
   * When this value changes and `defaultOpen` is true, force the section open.
   * Useful for new pending edits: a user may have collapsed an old batch, but
   * a fresh batch should surface itself immediately.
   */
  autoOpenKey?: string;
...
export function StatusCardSectionComponent({
  ...
  useEffect(() => {
    if (item.defaultOpen) setIsOpen(true);
  }, [item.autoOpenKey, item.defaultOpen]);
