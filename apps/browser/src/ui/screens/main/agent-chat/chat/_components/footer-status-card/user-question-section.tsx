...
import { IconHelpChatOutline18 } from '@clodex/icons';
...
function validateField(
  ...
  if (field.type === 'input') {
    ...
    if (
      field.validationFormat === 'date-time' &&
      strValue &&
      Number.isNaN(Date.parse(strValue))
    ) {
      return 'Invalid date and time';
    }
    if (field.validationFormat === 'uri' && strValue) {
      try {
        new URL(strValue);
      } catch {
        return 'Invalid URL';
