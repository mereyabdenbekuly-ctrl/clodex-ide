...
export const MessageUser = memo(
  function MessageUser({
    ...
    const canFork = !isWorking && !!msg.id && !!openAgent;

    const handleForkFromMessage = useCallback(
      async (event: MouseEvent<HTMLButtonElement>) => {
        event.stopPropagation();
        if (!canFork || !openAgent || isForking) return;

        setIsForking(true);
        try {
          const forkId = await forkAgent(openAgent, msg.id);
          setOpenAgent(forkId);
          await setLastOpenAgentId(forkId);
        } catch (error) {
          console.error('Failed to fork task from message:', error);
        } finally {
          setIsForking(false);
        }
      },
    ...
    const hasOpenedInternalPage = useMemo(() => {
      return activeTab?.url?.startsWith('clodex://internal/') ?? false;
    }, [activeTab?.url]);
    ...
    const hasVisibleBrowsingTab = useMemo(() => {
      ...
      return (
        ...
        !activeTab.url?.startsWith('clodex://internal/') &&
        !ownedByOtherAgent
      );
    ...
    const allAvailableElements = useMemo(() => {
      if (isEditing) {
        ...
        return combined.filter((el) => {
          if (!el.clodexId) return false;
          if (seen.has(el.clodexId)) return false;
          seen.add(el.clodexId);
          return true;
        });
      }
      return [];
      ...
      >
        ...
        >
          ...
          <div ref={measureRef} className="w-full">
            ...
            <div
              className={cn(
                'group/fork-message mt-2 flex w-full shrink-0 flex-row-reverse items-stretch justify-start gap-1',
            ...
            >
              ...
              <div
                className={cn(
                  ...
                  isEditing &&
                    'codex-composer w-full p-3 before:absolute before:inset-0 before:rounded-[inherit]',
                  isEditing && isEditDragOver && 'bg-hover-derived!',
                  !isEditing &&
                    'codex-message-user group wrap-break-word max-w-[85%] origin-bottom-right select-text px-3.5 py-2.5 font-normal text-[length:var(--codex-chat-font-size)] text-token-text-primary leading-6 transition-[background-color,border-color,box-shadow] last:mb-0.5 sm:max-w-184',
                  !isEditing &&
                    canEdit &&
                    'group/chat-message-user cursor-pointer active:scale-[0.998]',
                )}
              ...
              {!isEditing && canFork && (
                <Button
                  variant="ghost"
                  size="icon-xs"
                  className="mt-auto mb-1 shrink-0 opacity-0 transition-opacity focus-visible:opacity-100 group-hover/fork-message:opacity-100"
                  title="Fork task from this message"
