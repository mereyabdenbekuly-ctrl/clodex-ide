...
export function SidebarAuthFooter() {
  ...
  const avatarChar = profileLabel ? profileLabel[0]!.toUpperCase() : null;
  ...
  return (
    <div className="mt-2 flex shrink-0 flex-col gap-2 border-token-border-light border-t pt-2">
      <div className="flex flex-row items-center justify-between gap-2">
        {isAuthenticated ? (
          <button
            type="button"
            className="app-no-drag flex min-w-0 flex-1 flex-row items-center gap-2 rounded-lg px-1.5 py-1 hover:bg-token-list-hover-background active:bg-foreground/12"
            onClick={handleOpenAccount}
          >
            <div className="flex size-8 shrink-0 items-center justify-center rounded-full border border-token-border-light bg-token-main-surface-primary font-medium text-sm text-token-text-primary shadow-codex-sm">
              {avatarChar}
            </div>
            <div className="flex w-full min-w-0 flex-col items-start gap-px overflow-hidden">
              <span className="truncate font-medium text-sm text-token-text-primary leading-tight">
                {profileLabel ?? 'Clodex'}
              </span>
              <span className="truncate text-token-text-tertiary text-xs leading-tight">
                Signed in
              </span>
            </div>
          </button>
          ...
          <button
            type="button"
            className="app-no-drag flex h-9 min-w-0 flex-1 flex-row items-center gap-2 rounded-lg px-1.5 py-1 hover:bg-token-list-hover-background active:bg-foreground/12"
            onClick={handleOpenAccount}
          >
