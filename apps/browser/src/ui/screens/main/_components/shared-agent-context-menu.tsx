...
import { SessionContinuityDialog } from './session-continuity-dialog';
...
export const SharedAgentContextMenuHost = memo(
  function SharedAgentContextMenuHost({
    ...
    const continuityEnabled = resolveFeatureGate(
      'session-continuity',
      preferences.featureGates.overrides,
      releaseChannel,
    ).enabled;
    const [continuityAgentId, setContinuityAgentId] = useState<string | null>(
      null,
    );
    ...
    useEffect(() => {
      if (open || continuityAgentId) return;
      const t = setTimeout(() => {
        lastTargetRef.current = null;
      }, 300);
      return () => clearTimeout(t);
    }, [continuityAgentId, open]);
    ...
    return (
      <>
              ...
              >
                ...
                {continuityEnabled && (
                  <>
                    <AgentMenuItem
                      onClick={() => {
                        setContinuityAgentId(agentId);
                        onClose();
                      }}
                    >
                      <CloudUploadIcon className="size-3.5 shrink-0" />
                      <span>Continue in cloud</span>
                    </AgentMenuItem>
                    <AgentMenuItem
                      onClick={() => {
                        setContinuityAgentId(agentId);
                ...
                {onArchiveRequest && canDelete !== false && (
                  <AgentMenuItem
                    onClick={() => {
        ...
        <SessionContinuityDialog
          sessionId={continuityAgentId}
          onClose={() => setContinuityAgentId(null)}
        />
      </>
    );
  },
