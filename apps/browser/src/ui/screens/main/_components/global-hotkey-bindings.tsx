...
export function GlobalHotkeyBindings() {
  ...
  const { isOpen: isQuickTaskOpen } = useQuickTask();
  const globalHotkeysEnabled = !isCommandCenterOpen && !isQuickTaskOpen;
  ...
  const mainHotkeysEnabled = globalHotkeysEnabled && appScreenMode === 'main';
  ...
  useEffect(() => {
    if (!globalHotkeysEnabled || !projectsOpen) return;

    const handleProjectsEscape = (event: KeyboardEvent) => {
      if (event.key !== 'Escape' || event.defaultPrevented) return;
    ...
    window.addEventListener('keydown', handleProjectsEscape);
    return () => window.removeEventListener('keydown', handleProjectsEscape);
  }, [closeProjects, globalHotkeysEnabled, projectsOpen]);
  ...
  // Only applies when keyboard focus is on clodex UI chrome.
  ...
  const focusedPanel = activeTabId
    ? (tabUiState[activeTabId]?.focusedPanel ?? 'clodex-ui')
    : 'clodex-ui';
