...
} from '@clodex/stage-ui/components/overlay-scrollbar';
import {
...
import { useQuickTask } from '../../quick-task';
...
function AgentListGroupingToggle({
  ...
  const label = mode === 'workspace' ? 'Group by Project' : 'Group by Age';
  const options = [
    { value: 'age', label: 'Age' },
...
export function AgentsList() {
  ...
  const openProjects = useKartonProcedure((p) => p.appScreen.openProjects);
  ...
  const projectsOpen = useKartonState((s) => s.appScreen.mode === 'projects');
  ...
  const handleCreateMagusAgent = useCallback(() => {
    void track('chat-new-agent-clicked', { source: 'sidebar-magus-agent' });
    void closeProjects();

    agentCountAtCreateRef.current = agents.length;
    setPendingCreate(true);
    window.dispatchEvent(new Event('sidebar-chat-panel-opened'));
    const currentModelId = openAgentModelIdRef.current ?? 'gpt-5.5';
    const currentToolApprovalMode =
      openAgentToolApprovalModeRef.current ?? undefined;
    const paths = currentMountPathsRef.current;
    void createAgent(
  ...
  const handleOpenWorkspaceInFileManager = useCallback(
    (workspacePath: string) => {
      window.open(
        `clodex://reveal-file/${encodeURIComponent(workspacePath)}`,
        '_blank',
      );
    },
    [],
  ...
  const handleClick = useCallback(
    ...
    [closeProjects, resumeAgent, setOpenAgent, setLastOpenAgentId],
  );
    ...
    (id: string) => {
      ...
      void forkAgent(id)
        .then((forkId) => {
          ...
          return getAgentsHistoryList(0, fetchLimitRef.current);
        })
        .then(setHistoryList)
        .catch((err) => {
  ...
  const [pinnedHistoryList, setPinnedHistoryList] = useState<
    AgentHistoryEntry[]
  >([]);
  // Project list from the backend (`ChatPersistenceService.getProjects`).
  // Independent from `historyList` so that projects whose sessions are
  // outside the initial fetch window still surface in the sidebar.
      ...
      setChatProjects((current) =>
        current.map((project) => ({
          ...
          sessions: project.sessions.map((entry) =>
            patchTitle(entry, agentId, title),
          ),
        })),
      );
    };
    ...
    const handleAgentDeleted = (event: Event) => {
      ...
      setChatProjects((current) =>
        current
          .map((project) => ({
            ...project,
            sessions: removeDeletedAgent(project.sessions, agentId),
          }))
          // Drop projects whose last session was the one just deleted;
          // they no longer have anything to show.
  ...
  // Fetch the project list once on startup (after first agent is active).
  // Independent from `historyList` so projects whose sessions fall outside
  // the initial `INITIAL_HISTORY_FETCH` window are still surfaced — fixes
  // the orphan-project gap. Capped at the same 30-entry budget; further
  // sessions per project load on-demand when the project is expanded.
  const projectsFetchedRef = useRef(false);
  const projectsFetchLimitRef = useRef(INITIAL_HISTORY_FETCH);
  useEffect(() => {
    if (projectsFetchedRef.current) return;
    if (activeAgentIds.length === 0) return;
    projectsFetchedRef.current = true;
    getChatProjects(0, projectsFetchLimitRef.current)
      .then((projects) => {
  ...
  }, [activeAgentIds, getChatProjects]);
  ...
  const projectSessionGroups = useMemo(
    () => buildProjectSessionGroups(visibleUnpinnedAgents),
    [visibleUnpinnedAgents],
  );

  // Surface projects whose sessions fall outside the initial history fetch
  // window. Without this, a project whose 30+ sessions all sit past
  // `INITIAL_HISTORY_FETCH` would silently disappear from the "Group by
  // Project" view — orphan projects. We synthesise empty `ProjectSessionGroup`
  ...
  useEffect(() => {
    if (!enableWorktreeMetadataFetch || agentListGroupingMode !== 'workspace')
      return;
  ...
  }, [projectSessionGroups, scrollCardIntoView]);
  ...
  const renderProjectGroup = useCallback(
    (project: ProjectSessionGroup) => {
      const containsOpenAgent = project.agents.some(
        (agent) => agent.id === openAgent,
      );
      const collapsed =
        collapsedWorkspaceGroups.has(project.key) && !containsOpenAgent;

      return (
        <div key={project.key} className="contents">
          <WorkspaceGroupHeader
            label={project.label}
            depth={0}
            collapsed={collapsed}
            dotClassName={getSeverityDotClass(project.severity)}
            hasContent={project.agents.length > 0}
            collapsible={!containsOpenAgent}
            idleIcon={
              project.rootPath ? IconFolderOutline18 : IconSquareDashedOutline18
            }
            onToggle={() => toggleWorkspaceGroupCollapsed(project.key)}
            onOpenInFileManager={
              project.rootPath
                ? () => handleOpenWorkspaceInFileManager(project.rootPath!)
                : undefined
            }
            openInFileManagerIcon={IconFolderOutline18}
            onCreateAgent={
              project.rootPath
                ? () => handleCreateAgentForWorkspace(project.rootPath!)
                : handleCreateAgent
            }
          />
          {!collapsed &&
            project.agents.map((agent) =>
              renderAgentCard(agent, `${project.key}:${agent.id}`),
            )}
        </div>
      );
    },
  ...
  return (
    <div className="flex h-full flex-col group-data-[collapsed=true]:hidden">
      ...
      <div className="shrink-0 pt-1">
        <div className="flex gap-1">
          <Button
            variant="ghost"
            size="sm"
            className="group/new-agent relative h-9 min-w-0 flex-1 justify-start rounded-lg border border-token-border-light bg-token-main-surface-primary/55 px-2 text-start font-medium text-token-text-primary shadow-codex-sm hover:bg-token-list-hover-background"
          ...
          <Tooltip>
            <TooltipTrigger>
              <Button
                ...
                className="h-9 shrink-0 rounded-lg border border-token-border-light bg-token-main-surface-primary/35 text-token-text-secondary shadow-codex-sm hover:bg-token-list-hover-background hover:text-token-text-primary"
            ...
            <TooltipContent>
              <span className="flex items-center gap-2">
                Quick task
                <HotkeyCombo action={HotkeyActions.OPEN_QUICK_TASK} size="xs" />
              </span>
            </TooltipContent>
          </Tooltip>
          <Tooltip>
            <TooltipTrigger>
              ...
              >
                <IconFolderOutline18 className="size-4" />
              </Button>
            </TooltipTrigger>
          ...
          <Button
            variant="ghost"
            size="sm"
            className="h-9 shrink-0 gap-1 rounded-lg border border-token-border-light bg-token-main-surface-primary/35 px-2 font-medium text-token-text-secondary shadow-codex-sm hover:bg-token-list-hover-background hover:text-token-text-primary"
            title="New Magus agent"
            aria-label="New Magus agent"
            onClick={handleCreateMagusAgent}
          >
            <IconBrainOutline18 className="size-4 shrink-0" />
            <span>Magus</span>
          </Button>
        </div>
        <div className="mt-2 flex h-9 items-center gap-1.5 rounded-lg border border-token-border-light bg-token-main-surface-primary/35 px-2.5 transition-colors focus-within:border-token-border-default hover:bg-token-list-hover-background">
          <IconMagnifierOutline18 className="size-3.5 shrink-0 text-token-text-tertiary" />
          <input
            type="text"
            ...
            onChange={() => {}}
            className={cn(
              'w-full bg-transparent text-sm text-token-text-primary placeholder:text-token-text-tertiary',
              'outline-none',
            )}
          />
          <HotkeyCombo
      ...
      <OverlayScrollbar
        ...
        contentClassName="flex flex-col gap-px pl-0.5 pt-2 pb-3.5 pr-1.5"
        options={{
          overflow: { x: 'visible' },
          scrollbars: { theme: 'os-theme-clodex-subtle' },
        }}
        style={maskStyle}
      ...
      >
        <div className="flex shrink-0 items-center pt-0 pr-0 pb-1 pl-1.5">
          <div className="min-w-0 flex-1 truncate font-normal text-sidebar-foreground text-xs">
            {filteredPinnedAgents.length > 0 ? 'Pinned' : 'Tasks'}
          </div>
              ...
              {activePinnedDragAgent ? (
                <div className="[&_*]:!cursor-grabbing shadow-elevation-1">
                  <AgentCard
                    ...
                    type={activePinnedDragAgent.type}
