...
Last week focused on making clodex agents run more reliably when you're on the go, and on making worktrees a first-class part of the workflow.
...
Instead of one long list, agents can be split by the workspace they use. For Git repositories, clodex also separates agents by worktree, so parallel work stays easier to scan.

![The clodex agent sidebar grouped by workspace, with pinned agents, repository workspaces, and worktree sections.](/news/release-week-june-1-7-workspace-grouping.png)
...
The new grouping also adds better worktree actions. You can create an agent in an existing worktree or a new worktree with one click. When deleting a worktree from the sidebar, clodex can also clean up the agents that were using that workspace.

### Worktree setup scripts

clodex can now run setup scripts for managed worktrees.

![Worktree settings in clodex showing a setup script editor for new worktrees.](/news/release-week-june-1-7-worktree-setup-scripts.png)
...
![MiniMax M3 selected in the clodex model picker, with model details shown beside the list.](/news/release-week-june-1-7-minimax-m3.png)

MiniMax M3 is a frontier multimodal coding model with a long context window. It is now available alongside the other supported models for agentic coding work in clodex.
...
![The new coffee mug force-awake button in the bottom of the clodex sidebar.](/news/release-week-june-1-7-force-awake.png)
...
This week made clodex more reliable for parallel agent work.
