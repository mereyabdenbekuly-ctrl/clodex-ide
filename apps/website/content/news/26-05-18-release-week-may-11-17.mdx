...
clodex now remembers your last used **tool-call permission mode** and applies it to new agent chats automatically. If you work in Smart Approval, new agents start that way. No reconfiguring.
...
clodex was dropping earlier thinking content from **Anthropic Claude** and **Google Gemini** models under certain conditions. That's fixed. Claude and Gemini agents should now have an easier time staying consistent instead of circling back to the same issues.
...
- Closing clodex used to throw a crash report because LSP servers were not shutting down correctly. That's fixed — the app quits cleanly now.
