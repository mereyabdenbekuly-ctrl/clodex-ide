...
author: clodex team
type: releases
description: "A busy week of clodex releases: Persistent shell support, Support for Kimi K2.6, renamable agents, UI performance improvements and an improved context engine for the agent."
date: 2026-04-27
---
...
We shipped support for long-running shell sessions for clodex agents, allowing our agent to not only execute one-shot commands, but also own and control **long-running shell commands like dev servers, build processes** and much more. The agent is now also able to navigate interactive apps that require keyboard navigation.

This marks the first step of clodex integrating more terminal-based capabilities for both the agent and the user.

### First-Day Support for Kimi K2.6

We shipped support for **Kimi K2.6** on its release day, giving users access to **Moonshot.AI**'s latest and greatest model, combining strong agentic capabilities with a strong pricing model. We're excited to get feedback on what your experience with the latest Kimi model inside clodex looks like!

### Renamable agents

![An opened agent getting renamed by the user.](/news/release-week-april-20-26-renamable-agents.png)

As part of making it easier to keep control of all agent instances, clodex now allows you to rename individual agents. Simply click on the title of an agent in the active agents section to enable the edit mode.
