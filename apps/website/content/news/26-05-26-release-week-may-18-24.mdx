...
Last week focused on making clodex easier to use as an orchestration platform for multiple agents.
...
![Screenshot of the redesigned clodex browser tabs, including per-agent tabs and a pin action for global tabs.](/news/release-week-may-18-24-browsing-tabs.png)

The pin icon makes a tab global. Use it when a page should stay available outside one agent.

### clodex now recovers your tabs and selected agent after restart

clodex now stores which tabs were open before the last restart, plus which agent was selected.

On launch, both come back. The app state is closer to where you left it.

### UI zoom, more hotkeys, and less input clutter

You can now zoom the clodex UI in and out with the usual app shortcuts: **CMD/CTRL+-** to zoom out, **CMD/CTRL++** to zoom in, and **CMD/CTRL+0** to reset.
...
![Screenshot of Gemini 3.5 Flash selected in the clodex model picker.](/news/release-week-may-18-24-gemini-flash.png)

This adds another current Google model option for agentic coding work inside clodex.

### CMD+K opens the new control center

We added a new **CMD+K control center** for navigation and app actions.

![Screenshot of the clodex CMD+K control center showing agents, tabs, and settings.](/news/release-week-may-18-24-command-center.png)
