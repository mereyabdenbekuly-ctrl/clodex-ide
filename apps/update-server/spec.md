...
- Emergency rollout controls are configured through environment variables:
  - `UPDATE_BLOCKED_VERSIONS` is a comma-separated list of releases that must
    not be selected for updates or first-time downloads.
  - `UPDATE_CHANNEL_PINS` is a comma-separated `channel=version` map that holds
    a channel on one known-good release.
  - These controls are evaluated for every request against the cached GitHub
    release list, so changing them only requires an update-server restart.
  - Electron updates are forward-only. Blocking or pinning stops further
    rollout and protects clients on older versions, but it does not downgrade a
    client that already installed a bad release. Such clients require a newer
    forward-fix release.
...
The automated contract suite must prove all of the following:

- A normal rollout selects the newest compatible channel release.
- A blocked newest release falls back to the next known-good release.
- A client already running a blocked release is never offered a downgrade.
- A channel pin exposes only the pinned known-good release.
- Squirrel.Windows `RELEASES` entries retain their SHA-1 and size while their
  full-package URLs are rewritten to architecture-safe proxy URLs.
