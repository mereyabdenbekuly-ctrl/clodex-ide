...
function currentPolicy(): ReleasePolicy {
  return {
    blockedVersions: config.blockedVersions,
    channelPins: config.channelPins,
  };
}

export function getEligibleReleases(
  releases: readonly Release[],
  channel: Channel,
  policy: ReleasePolicy = currentPolicy(),
): Release[] {
  const blocked = new Set(policy.blockedVersions);
  const pin = policy.channelPins[channel];
  return releases.filter(
    (release) =>
      matchesChannel(release.parsedVersion, channel) &&
      !blocked.has(release.version) &&
      (!pin || release.version === pin),
  );
}
...
// Example: clodex-prerelease-1.0.0-beta001-arm64.dmg
...
// Example: clodex-prerelease-darwin-arm64-1.0.0-beta001.zip
...
// Example deb: clodex-prerelease_1.0.0.beta001_amd64.deb
// Example rpm: clodex-prerelease-1.0.0.beta001-1.x86_64.rpm
...
export async function findMacOSUpdateAsset(
  ...
  return selectMacOSUpdateAsset(releases, channel, arch, currentVersion);
}

export function selectMacOSUpdateAsset(
  releases: readonly Release[],
  channel: Channel,
  arch: string,
  currentVersion?: string,
  ...
  for (const release of getEligibleReleases(releases, channel, policy)) {
...
export async function findMacOSDownloadAsset(
  ...
  for (const release of getEligibleReleases(releases, channel)) {
 ...
 */
export function transformReleasesContent(
  content: string,
  release: Release,
  baseUrl: string,
  channel: string,
