...
import type { ShellExecutionBackend } from '../engine';
import type {
...
/**
 * Host-supplied Guardian hook. Returning `null` means the feature gate is
 * disabled and preserves the existing approval-mode/smart-approval flow.
 */
export interface GuardianApprovalDeps {
  assess: (
    input: SmartApprovalClassifyInput,
  ) => Promise<GuardianApprovalResult | null>;
  recordPendingApproval: (toolCallId: string, explanation: string) => void;
}
...
export const executeShellCommand = (
  ...
  guardianApproval?: GuardianApprovalDeps,
  capabilitySecurity?: ShellCapabilitySecurityDeps,
) => {
  return tool({
    ...
    needsApproval: async (
      ...
      if (input.kill) return await finishDecision(false);
      ...
      if (classifierCommand === '' && input.session_id) {
        return await finishDecision(false);
      }
      ...
      const shellTail = input.session_id
        ...
        : '';
      const classificationInput: SmartApprovalClassifyInput = {
      ...
      if (guardianApproval) {
        let guardianResult: GuardianApprovalResult | null;
        try {
          guardianResult = await guardianApproval.assess(classificationInput);
        } catch {
          guardianApproval.recordPendingApproval(
            toolCallId,
            'Guardian assessment failed. Approving manually to stay safe.',
          );
          return await finishDecision(true);
        }

        if (guardianResult) {
          if (guardianResult.decision === 'deny') {
            throw new Error(
              `Guardian denied action: ${guardianResult.explanation}`,
            );
          }
          if (
            guardianResult.irreversible ||
            guardianResult.decision === 'escalate'
          ) {
            guardianApproval.recordPendingApproval(
              toolCallId,
          ...
          // Guardian approval can bypass only the smart classifier for a
          // reversible low-risk action. An explicit alwaysAsk preference
          // remains authoritative.
          return await finishDecision(getToolApprovalMode() === 'alwaysAsk');
        }
      }

      const mode = getToolApprovalMode();
      if (mode === 'alwaysAllow') return await finishDecision(false);
      if (mode === 'alwaysAsk') return await finishDecision(true);
      ...
      if (!smartApproval) return await finishDecision(true);
      ...
      try {
        result = await smartApproval.classify(classificationInput);
      } catch {
    ...
    execute: async (
      ...
      try {
        ...
        const currentCwd = shellService.getSessionCurrentCwd(params.session_id);
        const isPoll = (params.command ?? '') === '';
        const cwdPrefix =
          isPoll || !currentCwd
            ? ''
            : toClassifierCwdPrefix(currentCwd, getMountedPaths);
