...
describe('SessionLogger', () => {
  ...
  it('stores protected append segments while preserving synchronous tail reads', async () => {
    const logPath = makeTmpLogPath('protected.shell.log');
    const storage = new ProtectedFileStorage(
      new AeadDataProtection(randomBytes(32)),
      { chunkSize: 4096 },
    );
    const context = 'shell-logs/agent/protected.shell.log';
    const logger = new SessionLogger(logPath, 120, 30, {
      storage,
      context,
    });

    logger.append('secret first line\n');
    logger.flush();
    logger.append('secret second line\n');
    expect(logger.getTail(100)).toBe('secret first line\nsecret second line\n');
    logger.close();
    await logger.drain();

    expect(fs.readFileSync(logPath, 'utf-8')).not.toContain('secret');
    const appendStore = new ProtectedAppendFileStorage(
      storage,
      logPath,
      context,
    );
    expect((await appendStore.readFile()).toString('utf-8')).toBe(
      'secret first line\nsecret second line\n',
    );
  });

  it('surfaces protected persistence failures during drain', async () => {
    const logger = new SessionLogger(
      makeTmpLogPath('failing.shell.log'),
      120,
      30,
      {
        storage: {
        ...
        context: 'shell-logs/agent/failing.shell.log',
      },
    );

    logger.append('must be durable\n');
    logger.close();
    await expect(logger.drain()).rejects.toThrow(
      'Failed to persist shell session log',
    );
  });
});
