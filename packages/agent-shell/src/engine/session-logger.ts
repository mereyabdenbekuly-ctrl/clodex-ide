...
const MAX_TAIL_CACHE_CHARS = 256 * 1024;

export interface ProtectedSessionLogOptions {
...
export class SessionLogger {
  ...
  private _tailCache = '';
  private _protectedLog: ProtectedAppendFileStorage | null;
  private _persistenceQueue = Promise.resolve();
  private _persistenceError: unknown = null;
  ...
  constructor(
    ...
    protectedLog?: ProtectedSessionLogOptions,
  ) {
    ...
    this._protectedLog = protectedLog
      ? new ProtectedAppendFileStorage(
          protectedLog.storage,
          filePath,
          protectedLog.context,
        )
      : null;
  }
  ...
  append(data: string): void {
    ...
    this._buffer += data;
    this._tailCache = `${this._tailCache}${data}`.slice(-MAX_TAIL_CACHE_CHARS);
  ...
  getTail(maxChars: number): string {
    this.flush();
    if (this._protectedLog) {
      return this._tailCache.slice(-maxChars);
    }
    try {
  ...
  public async drain(): Promise<void> {
    ...
    try {
      await this._protectedLog?.drain();
    } catch (error) {
      this._persistenceError ??= error;
    }
    if (this._persistenceError) {
      throw new Error('Failed to persist shell session log', {
        cause: this._persistenceError,
      });
    }
  }
