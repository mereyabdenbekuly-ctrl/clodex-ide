...
describe('sanitizeEnv', () => {
  ...
  it('sets CLODEX_SHELL marker', () => {
    const env = sanitizeEnv();
    expect(env.CLODEX_SHELL).toBe('1');
  });
  ...
  it('strips the inherited shell-integration guard so each PTY can re-source', () => {
    ...
    process.env.__CLODEX_SHELL_INTEGRATION = '1';

    const env = sanitizeEnv();

    expect(env.__CLODEX_SHELL_INTEGRATION).toBeUndefined();
  });
  ...
  describe('forAgent option', () => {
    it('applies agent-specific env by default (CLODEX_SHELL, HISTFILE)', () => {
      const env = sanitizeEnv();
    ...
    it('omits agent-specific env when forAgent is false', () => {
      ...
      expect(env.CLODEX_SHELL).toBeUndefined();
