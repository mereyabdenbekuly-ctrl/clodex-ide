...
/**
 * Environment variables injected by the clodex host process (via Vite
...
export function sanitizeEnv(
  ...
  for (const [key, value] of Object.entries(base)) {
    ...
    if (key === '__CLODEX_SHELL_INTEGRATION') continue;
  ...
  // behavior and do not carry the CLODEX_SHELL marker.
  if (options?.forAgent !== false) {
    ...
    // clodex shell, or the app was launched from a terminal that had
    // CLODEX_SHELL set). Without this, they'd pass through unchanged.
    delete env.HISTFILE;
    delete env.HISTSIZE;
    delete env.SAVEHIST;
    delete env.CLODEX_SHELL;
  }
