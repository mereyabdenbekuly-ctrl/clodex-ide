...
describe('injectBoundaryToken', () => {
  it('replaces all shell integration boundary placeholders', () => {
    ...
    expect(
      injectBoundaryToken(
        'BoundaryToken="__CLODEX_BOUNDARY_TOKEN__"; D=__CLODEX_BOUNDARY_TOKEN__',
        token,
      ),
  ...
  it('removes placeholders when no parser token is available', () => {
    expect(injectBoundaryToken('__CLODEX_BOUNDARY_TOKEN__', undefined)).toBe(
      '',
    );
  });
});
 ...
 * macOS sandboxed apps (e.g. clodex) may block posix_spawnp.
 */
...
describe('shell integration scripts', () => {
  ...
  it('integration guard is set but not exported (no leak into child shells)', () => {
    ...
    for (const script of [BASH_INTEGRATION, ZSH_INTEGRATION]) {
      expect(script).toContain('__CLODEX_SHELL_INTEGRATION=1');
      expect(script).not.toContain('export __CLODEX_SHELL_INTEGRATION');
    }
  });
});
