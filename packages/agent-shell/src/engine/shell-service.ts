...
export class ShellService extends DisposableService {
  ...
  async initialize() {
    ...
    if (this.shell) {
      ...
      this.sessionManager = new SessionManager(
        this.shell,
        (agentId) => this.getShellLogsDir(agentId),
        undefined,
        undefined,
        this.protectedFiles,
      );
  ...
  protected async onTeardown(): Promise<void> {
    ...
    const logDrain = sessionManager?.killAllAndDrain();
    ...
    await logDrain;
    this.logger.debug(
      `[ShellService] Teardown complete (killed ${killedCount} PTY session(s), logs drained)`,
    );
  }
}
