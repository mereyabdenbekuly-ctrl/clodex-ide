...
export function injectBoundaryToken(
  ...
  return script.replaceAll('__CLODEX_BOUNDARY_TOKEN__', boundaryToken ?? '');
}
...
if [ -n "$__CLODEX_SHELL_INTEGRATION" ]; then return 0 2>/dev/null || true; fi
...
__CLODEX_SHELL_INTEGRATION=1
{ unset HISTFILE; } 2>/dev/null
HISTSIZE=0 2>/dev/null
HISTFILESIZE=0 2>/dev/null
set +o history 2>/dev/null
__clodex_command_executed=0
__clodex_boundary_token="__CLODEX_BOUNDARY_TOKEN__"
__clodex_file_uri_path() {
  local path="$1"
  local encoded=''
  local i ch hex
  local LC_ALL=C
...
__clodex_emit_cwd() {
  printf '\\033]7;file://%s%s\\007' "\${HOSTNAME:-localhost}" "$(__clodex_file_uri_path "$PWD")"
}
__clodex_prompt_command() {
  local exit_code=$?
  if [ "$__clodex_command_executed" = "1" ]; then
    printf '\\033]133;D;%d;%s\\007' "$exit_code" "$__clodex_boundary_token"
    __clodex_command_executed=0
  fi
  __clodex_emit_cwd
...
__clodex_pre_exec() {
  if [ "$BASH_COMMAND" = "__clodex_prompt_command" ]; then return; fi
  __clodex_command_executed=1
  printf '\\033]133;B\\007'
  printf '\\033]133;C\\007'
}
trap '__clodex_pre_exec' DEBUG
if [ -z "$PROMPT_COMMAND" ]; then
  PROMPT_COMMAND='__clodex_prompt_command'
else
  PROMPT_COMMAND="__clodex_prompt_command;\${PROMPT_COMMAND}"
fi
__clodex_emit_cwd
printf '\\033]133;A\\007'
`.trim();
...
__clodex_emit_cwd() {
  printf '\\033]7;file://%s%s\\007' "\${HOST:-\${HOSTNAME:-localhost}}" "$(__clodex_file_uri_path "$PWD")"
}
...
if (( $+functions[add-zsh-hook] )); then
  add-zsh-hook precmd __clodex_precmd
  add-zsh-hook preexec __clodex_preexec
else
  precmd_functions+=(__clodex_precmd)
  preexec_functions+=(__clodex_preexec)
fi
__clodex_emit_cwd
printf '\\033]133;A\\007'
`.trim();
...
if ((Test-Path variable:global:__ClodexState) -and $null -ne $Global:__ClodexState.OriginalPrompt) {
  return
}
if ($ExecutionContext.SessionState.LanguageMode -ne "FullLanguage") {
  return
}
$Global:__ClodexState = @{
  OriginalPrompt = $function:Prompt
  LastHistoryId  = -1
  IsInExecution  = $false
  BoundaryToken  = "__CLODEX_BOUNDARY_TOKEN__"
}
function Global:Prompt() {
  ...
  $Result = ""
  if ($Global:__ClodexState.LastHistoryId -ne -1 -and ($Global:__ClodexState.HasPSReadLine -eq $false -or $Global:__ClodexState.IsInExecution -eq $true)) {
    $Global:__ClodexState.IsInExecution = $false
    if ($LastHistoryEntry.Id -eq $Global:__ClodexState.LastHistoryId) {
      $Result += "$([char]0x1b)]133;D;0;$($Global:__ClodexState.BoundaryToken)\`a"
    } else {
      $Result += "$([char]0x1b)]133;D;$FakeCode;$($Global:__ClodexState.BoundaryToken)\`a"
    }
  }
  ...
  $Result += $Global:__ClodexState.OriginalPrompt.Invoke()
  $Result += "$([char]0x1b)]133;B\`a"
  $Global:__ClodexState.LastHistoryId = $LastHistoryEntry.Id
  return $Result
}
$Global:__ClodexState.HasPSReadLine = $false
if (Get-Module -Name PSReadLine) {
  Set-PSReadLineOption -HistorySaveStyle SaveNothing
  $Global:__ClodexState.HasPSReadLine = $true
  $Global:__ClodexState.OriginalPSConsoleHostReadLine = $function:PSConsoleHostReadLine
  function Global:PSConsoleHostReadLine {
    $CommandLine = $Global:__ClodexState.OriginalPSConsoleHostReadLine.Invoke()
    $Global:__ClodexState.IsInExecution = $true
...
export class SessionManager {
  ...
  private readonly pendingLoggerDrains = new Set<Promise<void>>();
  private readonly loggerDrainErrors: unknown[] = [];
    ...
    const session: PtySession = {
      ...
      logger: this.getShellLogsDir
        ? new SessionLogger(
            ...
            this.protectedFiles
              ? {
                  storage: this.protectedFiles,
                  context: protectedFileContext.shellLog(
                    agentInstanceId,
                    `${sessionId}.shell.log`,
                  ),
                }
  ...
  /**
   * Synchronously deactivates every PTY, then waits for all protected log
   * segments/manifests to reach durable storage. The tracked set also covers
   * sessions removed earlier by destroyAgent(), so graceful app shutdown
   * cannot lose their final queued append.
   */
  async killAllAndDrain(): Promise<void> {
    this.killAll();
    await Promise.all([...this.pendingLoggerDrains]);
    if (this.loggerDrainErrors.length > 0) {
      const errors = this.loggerDrainErrors.splice(0);
      throw new AggregateError(errors, 'Failed to drain shell session logs');
    }
  }
  ...
  private deactivateSession(session: PtySession): void {
    ...
    if (session.logger) {
      ...
      this.trackLoggerDrain(session.logger);
    }
  ...
  private trackLoggerDrain(logger: SessionLogger): void {
    let tracked: Promise<void>;
    tracked = logger
      .drain()
      .catch((error: unknown) => {
        this.loggerDrainErrors.push(error);
      })
      .finally(() => {
        this.pendingLoggerDrains.delete(tracked);
      });
    this.pendingLoggerDrains.add(tracked);
  }
