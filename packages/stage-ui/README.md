...
This is the canonical guide for building and designing UIs for clodex. It covers all you need to know.
...
Same for the components - but with the use of ts aliases (e.g. `import { Button } from '@clodex/stage-ui/components/button';`). `stage-ui`-code will not be built/ bundled.
...
```bash
pnpm --filter @clodex/stage-ui generate:theme-colors
```
**To generate a TextMate json for shiki based on our palette once, run:**
```bash
pnpm --filter @clodex/stage-ui generate:themes
```
**Run the dev server to regenerate both on changes in our palette:**
```bash
pnpm --filter @clodex/stage-ui dev:themes
```
...
We created the tailwind plugin `@clodex/tailwindcss-color-modifiers` which lets us use *any* defined color with modified lightness/chroma/hue/alpha channels.
...
Start it via `pnpm --filter clodex storybook`.
