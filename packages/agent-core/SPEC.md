...
This document is the canonical design and implementation brief for extracting the current clodex agent system out of the Electron browser app and into a standalone agent package.
...
- allows Electron to use current Clodex proxy/BYOK logic;
...
- all tool input/output Zod schemas and inferred types (`writeToolSchema`, `multiEditToolSchema`, ...), `ClodexToolSet`, `ToolName`, `ToolOutputDiff`, `WithDiff`
...
- `ModelCapabilities`, `ModalityConstraint`, `ModelSettings`, `ClodexProviderOptions`
...
- agents DB at the current `<userData>/clodex/agents/instances.sqlite` location with unchanged schema
- diff-history DB at the current `<userData>/clodex/diff-history/data.sqlite` location with unchanged schema
- attachment blobs at `<userData>/clodex/agents/<agentId>/data-attachments/<attachmentId>` with unchanged raw byte layout
...
| 3 | Workspace-local `.clodex` skills | `<mount>/.clodex/skills/` | **Package-owned**, via mount registry |
| 4 | Workspace-local `.agents` skills | `<mount>/.agents/skills/` | **Package-owned**, via mount registry |
| 5 | User-global `.clodex` skills | `~/.clodex/skills/` | Host-provided root (delivery mechanism TBD in constructive pass) |
...
3. Workspace-local `.clodex/skills` (source 3)
4. Workspace-local `.agents/skills` (source 4)
5. User-global `.clodex/skills` (source 5)
...
- **Host owns:**
  ...
  - Knowledge of OS conventions for user-global roots (`~/.clodex/skills`, `~/.agents/skills`). Delivery mechanism (pull via `HostPaths` or push via a registration API) is TBD in the constructive pass.
...
async function resolveSkills(ctx: AgentContext): Promise<ResolvedSkill[]> {
  ...
  for (const mount of mounts) {
    out.push(...await scanSkillRoot(join(mount.path, '.clodex', 'skills'), {
...
- **D-KB-2. Package-pure initial state.** `@clodex/agent-core` exports a pure `createInitialAgentSystemState()` factory that returns empty `agents.instances` and empty `toolbox` maps. The host seeds per-agent entries into the store when it hydrates agents (later phase). No Karton-shape import in the factory.
...
const tools: ToolSet = {
  ...makeUniversalTools({
    ...
    diffHistoryService,
    mutations,
  }), // from @clodex/agent-core; uses the package fs proxy
...
The package-side dependency surface is intentionally narrow: `agentInstanceId`, `HostPaths`, the core mount-manager lookup interface, optional static mounts, `DiffHistoryService`, optional logger, and optional file-mutation callbacks for host LSP synchronization. No browser-only service (`ClientRuntimeNode`, Karton, sandbox, shell, window layout, or LSP service) crosses into `@clodex/agent-core`.
...
Clodex currently has richer mention/resource metadata, including:
...
Phase 4 complete: all listed services (minus `asset-cache`, which stays host-owned until the clodex-auth tool-capability surface is formalized in Phase 7) now live inside `@clodex/agent-core`. Relocations:
...
- `packages/agent-core/src/services/agent-persistence/` — `AgentPersistenceDB.create({ host, logger, dataProtection? })` resolves `host.paths.agentDbPath()` internally; sensitive JSON fields use the optional host-owned AEAD capability while `StoredAgentInstance` / `NewStoredAgentInstance` / `ToolApprovalMode` schemas ship from core.
...
No shims. Every `@shared/ast/*`, `@shared/plan-parsing`, `@shared/plan-ownership`, `@shared/log-ownership`, `@/services/file-read-cache`, `@/services/processed-image-cache`, `@/utils/attachment-blobs`, `@/agents/shared/base-agent/file-read-transformer`, and `@/agents/shared/base-agent/image-processor` import in `apps/browser/src` was rewritten to the matching `@clodex/agent-core/...` subpath export, and the host-side originals were deleted. All `node:fs` / `node:fs/promises` access inside the migrated modules routes through the `@clodex/agent-core/fs` proxy so the `noRestrictedImports` Biome guard stays enforceable.
...
The Karton mirror for `toolbox[agentId].workspace.mounts` continues through `setAgentMounts` (writing into the shared `AgentStore`) + `AgentCoreBridge`; `pages-api` and `window-layout` observe the unchanged shape. Tests: 181 in `@clodex/agent-core`, 841 in `apps/browser`.
