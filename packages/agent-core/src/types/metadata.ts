...
export const reasoningSignatureSourceSchema = z.object({
  providerMode: z.enum(['clodex', 'official', 'custom']),
  provider: z.string(),
...
export const metadataSchema = z.object({
  ...
  /**
   * Host-side routing hint. When true, browser hosts should route the user
   * turn through the DynamicSwarmOrchestrator instead of the regular agent.
   */
  swarmMode: z.boolean().optional(),
  /**
   * Optional host-side variant for Swarm submissions. `battle` forces the
   * debate/arbitration workflow instead of letting triage choose quietly.
   */
  swarmModeVariant: z.enum(['standard', 'battle']).optional(),
  /**
   * Host-independent execution routing hint. `cloud` must never silently
   * fall back to local execution after selection.
   */
  executionTarget: z.enum(['local', 'cloud']).optional(),
  /**
   * Requests a session-wide cloud handoff. Unlike ordinary per-turn cloud
   * execution, the snapshot is built from all mounted workspace roots and
   * does not inherit the latest message's pathReferences.
   */
  cloudHandoffScope: z.enum(['session-workspaces']).optional(),
  /**
   * Versioned, durable safe-point checkpoint prepared immediately before a
   * Session Teleport cloud handoff.
   */
  sessionCheckpoint: z.unknown().optional(),
  /**
   * Durable deduplication cursor for cloud UI chunks replayed after a host
   * restart. Contains only opaque execution identity and sequence.
   */
  cloudReplay: z
    .object({
   ...
   * Host UIs may use it to attach run-specific artifacts below the message.
   */
  swarmResultRunId: z.string().optional(),
  /**
   * Host UI hint: render pending/edit-summary file diffs as a message-level
   * artifact below this assistant message.
   */
