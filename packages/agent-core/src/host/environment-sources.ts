...
export interface ResolvedSkillEntry {
   ...
   * (e.g. `wXXX/.clodex/skills/foo`, `plugins/github/SKILL.md`).
   * Absent for builtins.
   */
  skillPath?: string;
...
/** Host-level runtime context surfaced into the agent prompt. */
export interface RuntimeContextSnapshot {
  /** User-visible OS/platform label, e.g. `darwin`, `linux`, `win32`. */
  osName: string;
  ...
  currentTime: string;
  /**
   * Currently focused editor file, if the host can resolve one. Prefer
   * mount-prefixed/project-relative paths over absolute paths.
   */
...
export interface HostEnvironmentSources {
   ...
   * (e.g. `~/.clodex/skills`, `~/.agents/skills`). The host is
  ...
  /**
   * Optional runtime context for each turn. Hosts that expose an editor
   * surface should include the currently focused file when known.
   */
  getRuntimeContext?(
    agentInstanceId: string,
  ): RuntimeContextSnapshot | Promise<RuntimeContextSnapshot>;
}
