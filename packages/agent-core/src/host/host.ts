...
import {
  ...
  type ProtectedFileStorageOptions,
} from './protected-files';
import type { Logger } from './logger';
import type { HostModels } from './models';
import type { HostPaths } from './paths';
import type { TelemetrySink } from './telemetry';
import type { EvidenceMemoryService } from '../services/evidence-memory';
 ...
 * the alias to a concrete URL. agent-core bakes in the clodex
...
export interface AgentHostConfig {
  ...
  logger: Logger;
  /**
   * Optional host-owned encryption capability for sensitive persisted data.
   * Desktop hosts should unlock/load this before constructing AgentHost.
   * Headless/test hosts may omit it and retain legacy plaintext persistence.
   */
  dataProtection?: DataProtection;
  /**
   * Optional protected-file implementation. When omitted but dataProtection
   * is present, AgentHost constructs the standard chunked implementation.
   */
  protectedFiles?: ProtectedFileStorage;
  protectedFileOptions?: ProtectedFileStorageOptions;
   ...
   * `'.clodex/WORKSPACE.md'`). Defaults to `.clodex/WORKSPACE.md`
   * when omitted.
   */
...
/**
 * Capability seam between `@clodex/agent-core` and its host
...
export class AgentHost {
  ...
  readonly dataProtection: DataProtection | undefined;
  readonly protectedFiles: ProtectedFileStorage | undefined;
  readonly telemetry: TelemetrySink | undefined;
  readonly desktop: HostDesktop | undefined;
  /**
   * Optional append-only evidence ledger. Persistence binds this after its
   * asynchronous database migration completes. Agents must treat it as
   * best-effort and never block their primary execution path on availability.
   */
  ...
  constructor(cfg: AgentHostConfig) {
    ...
    this.dataProtection = cfg.dataProtection;
    this.protectedFiles =
      cfg.protectedFiles ??
      (cfg.dataProtection
        ? new ProtectedFileStorage(cfg.dataProtection, cfg.protectedFileOptions)
        : undefined);
  ...
  /**
   * Late-bind the evidence ledger created by `AgentCorePersistence`.
   *
   * `AgentHost` is assembled before asynchronous persistence services, so
   * this setter keeps construction ordering explicit without introducing a
   * second host object or a global singleton.
   */
  bindEvidenceMemory(service: EvidenceMemoryService | undefined): void {
    this.evidenceMemory = service;
  }

  /**
   * Mount-relative path to the WORKSPACE.md project memo. Always
   * returns a value; falls back to `.clodex/WORKSPACE.md` when the
   ...
   * (`@clodex/agent-core/file-read-transformer`) for generic
