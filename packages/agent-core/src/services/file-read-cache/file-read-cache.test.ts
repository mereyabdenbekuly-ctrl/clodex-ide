...
import { AeadDataProtection, isDataProtectionEnvelopeString } from '../../host';
...
describe('FileReadCacheService – protected persistence', () => {
  it('stores ciphertext and returns decrypted content', async () => {
    const url = await freshDbUrl();
    const protection = new AeadDataProtection(randomBytes(32));
    const cacheKey = sha256('protected-file-cache');
    const plaintext = 'private transformed model context';
    const svc = await FileReadCacheService.createWithUrl(
      url,
      noopLogger,
      protection,
    ...
    await svc.set(cacheKey, plaintext, plaintext.length);
    expect((await svc.get(cacheKey))?.content).toBe(plaintext);
    await svc.teardown();

    const client = createClient({ url });
    try {
      const result = await client.execute({
        sql: 'SELECT content FROM file_read_cache WHERE content_hash = ?',
        args: [cacheKey],
      });
      const stored = String(result.rows[0]?.content);
  ...
  it('migrates legacy plaintext rows and rejects a wrong key', async () => {
    const url = await freshDbUrl();
    const cacheKey = sha256('legacy-file-cache');
    const plaintext = 'legacy private transformed content';
    const legacy = await FileReadCacheService.createWithUrl(url, noopLogger);
    await legacy.set(cacheKey, plaintext, plaintext.length);
    await legacy.teardown();

    const protection = new AeadDataProtection(randomBytes(32));
    const migrated = await FileReadCacheService.createWithUrl(
      url,
      noopLogger,
      protection,
    );
    expect((await migrated.get(cacheKey))?.content).toBe(plaintext);
    await migrated.teardown();

    await expect(
      FileReadCacheService.createWithUrl(
        url,
        noopLogger,
