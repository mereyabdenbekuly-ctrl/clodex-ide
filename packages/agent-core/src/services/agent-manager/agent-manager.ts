...
} from '../../agents/session-checkpoint';
...
export class AgentManager extends DisposableService {
  ...
  public constructor(options: AgentManagerOptions) {
    ...
    this.stepExecutor = execution?.stepExecutor;
    this.agentStore = state.store;
    this.persistenceDb = storage.persistenceDb;
    this.chatPersistence =
      storage.chatPersistence ??
      new ChatPersistenceService({
        persistenceDb: storage.persistenceDb,
        enrichHistoryEntries: hooks?.enrichHistoryEntries,
      });
  ...
  /**
   * Establishes a durable idle boundary before Session Teleport packages the
   * workspace and dispatches a cloud turn. It never stops or suspends the
   * local agent; failures therefore leave local execution authoritative.
   */
  public async prepareSessionCheckpoint(
    ...
    if (!live) {
      ...
      if (!stored) {
        throw new AgentSessionCheckpointSafePointError('agent-not-found');
      }
      const flushedAt = new Date().toISOString();
      return {
        agentStateFlushedAt: flushedAt,
        memoryFlushedAt: flushedAt,
        wasLive: false,
      };
    }
  ...
  public async replayRecoveredUiChunk(
    ...
    const agent = await this.resumeAgent(instanceId);
    if (!agent) {
      throw new AgentSessionCheckpointSafePointError('agent-not-found');
    }
    const result = await agent.replayRecoveredUiChunk(input);
    await this.agentStore.whenSettled();
    return result;
  }
  ...
  public async finishRecoveredUiReplay(
    instanceId: string,
    input: Parameters<BaseAgent<any, any>['finishRecoveredUiReplay']>[0],
  ): Promise<void> {
    const agent = await this.resumeAgent(instanceId);
    if (!agent) {
      throw new AgentSessionCheckpointSafePointError('agent-not-found');
    }
    await agent.finishRecoveredUiReplay(input);
    await this.agentStore.whenSettled();
  }
   ...
   * `@clodex/agent-core/env/adapters` — so `AgentManager` itself stays
  ...
  private registerCommandHandlers(): () => void {
    this.wrapAgentRpc(
      ...
      async (
        ...
        if (agentType === AgentTypes.MAGUS) {
          initialState.title = 'Magus Agent';
        }
    ...
    this.wrapAgentRpc(
      'agents.fork',
      async (instanceId: string, throughMessageId?: string) => {
        return await this.forkAgent(instanceId, throughMessageId);
      },
    );
    this.wrapAgentRpc('agents.archive', async (instanceId: string) => {
      await this.archiveAgent(instanceId);
      const updated = await this.persistenceDb.setAgentArchived(
        instanceId,
        true,
      );
      if (!updated) {
    ...
    this.wrapAgentRpc('agents.unarchive', async (instanceId: string) => {
      const updated = await this.persistenceDb.setAgentArchived(
        instanceId,
        false,
      );
    ...
    this.wrapAgentRpc(
      ...
      async (
        ...
        if (trimmed.length < 1 || trimmed.length > 500) {
          throw new Error('Goal objective must contain 1–500 characters');
        }
        if (
          ...
          throw new Error(
            'Goal token budget must be an integer between 1 and 10,000,000',
          );
        }
        if (
          ...
          (!Number.isInteger(timeBudgetSeconds) ||
            timeBudgetSeconds < 1 ||
            timeBudgetSeconds > 31_536_000)
        ) {
          throw new Error(
            'Goal time budget must be an integer between 1 and 31,536,000 seconds',
          );
        }
        ...
        const now = Date.now();
        const currentGoal = state.goal ?? null;
        setTaskGoal(this.agentStore, instanceId, {
          goal: currentGoal
            ? {
                ...currentGoal,
                objective: trimmed,
                tokenBudget: tokenBudget ?? null,
                timeBudgetSeconds: timeBudgetSeconds ?? null,
                accumulatedActiveMs: currentGoal.accumulatedActiveMs ?? 0,
                activeStartedAt:
                  currentGoal.status === 'active'
                    ? (currentGoal.activeStartedAt ?? now)
                    : null,
                updatedAt: now,
              }
            : {
                ...
                tokenBudget: tokenBudget ?? null,
                timeBudgetSeconds: timeBudgetSeconds ?? null,
                startedUsedTokens: state.usedTokens,
    ...
    this.wrapAgentRpc(
      ...
      async (instanceId: string, status: TaskGoalStatus) => {
        if (!['active', 'completed', 'cancelled', 'blocked'].includes(status)) {
          throw new Error(`Unsupported goal status: ${status}`);
        }
        const goal =
          this.agentStore.get().agents.instances[instanceId]?.state.goal;
        if (!goal) {
          throw new Error(`Agent with instance id ${instanceId} has no goal`);
        }
        setTaskGoalStatus(this.agentStore, instanceId, {
          status,
          updatedAt: Date.now(),
        });
        await this.persistAgentState(instanceId);
    ...
    this.wrapAgentRpc('agents.clearGoal', async (instanceId: string) => {
      const state = this.agentStore.get().agents.instances[instanceId]?.state;
      if (!state) {
        throw new Error(`Agent with instance id ${instanceId} not found`);
      }
      clearTaskGoal(this.agentStore, instanceId);
      await this.persistAgentState(instanceId);
    });
    ...
    this.wrapAgentRpc(
      ...
      async (
        ...
        return await this.getAgentsHistoryList(
          offset,
          limit,
          searchString,
          archived,
    ...
    this.wrapAgentRpc(
      ...
      async (
        ...
        return await this.getChatProjects(
          offset,
          limit,
          searchString,
          archived,
    ...
    this.wrapAgentRpc(
      ...
      async (ids: string[]) => {
        return await this.chatPersistence.getSessionsByIds(ids);
      },
    );
    this.wrapAgentRpc(
    ...
    this.wrapAgentRpc('agents.getStoredInstance', async (agentId: string) => {
      ...
      return {
        ...
        forkedFromAgentId: row.forkedFromAgentId,
  ...
  protected async onTeardown(): Promise<void> {
    ...
    // Tail-flush every active agent's envelope before its teardown so the
    // final in-flight stream chunk reaches `agentMessages`. `persistAgentState`
    // is idempotent: the divergence detector in `AgentPersistenceDB` is a
    // no-op when the envelope already matches the last DB row, so an
    // idle-agent quit stays at near-zero cost.
    //
    // Observability: timing is captured both per-agent (debug) and as a
    // summary (info + telemetry). The summary line is the leading edge
    // of "is shutdown getting slower over releases?" — pair with the
    // budget-expiry warn in `apps/browser/src/backend/main.ts` to spot
    // tail-flush overruns.
    const t0 = Date.now();
    let flushed = 0;
    let failed = 0;
    for (const [instanceId, agent] of this.activeAgents) {
      ...
      try {
        ...
        failed += 1;
        this.logger.warn(
          `[AgentManager] Final saveState failed for ${instanceId}`,
          { error },
        );
      } finally {
        const dt = Date.now() - agentStart;
        this.logger.debug(
          `[AgentManager] Flushed agent ${instanceId} in ${dt}ms (${agentOk ? 'ok' : 'failed'})`,
        );
      }
      if (agentOk) flushed += 1;
      try {
        await agent.onTeardown();
      } catch (error) {
        this.logger.warn(
          `[AgentManager] agent.onTeardown failed for ${instanceId}`,
          { error },
        );
      }
    }
    ...
    this.logger.info(
      `[AgentManager] Tail-flush complete: ${flushed} agents in ${totalMs}ms (${failed} failed)`,
    );
    this.host.telemetry?.capture('agent-shutdown-tail-flushed', {
      agent_count: flushed,
      failed_count: failed,
      duration_ms: totalMs,
    });
  ...
  public async resumeAgent(instanceId: string) {
    ...
    if (agent.archivedAt) {
      throw new Error(
        `Agent with instance id ${instanceId} is archived and must be restored before it can be resumed`,
      );
    }
  ...
  private async forkAgent(
    instanceId: string,
    throughMessageId?: string,
  ): Promise<string> {
    const live = this.activeAgents.get(instanceId);
    const liveState = this.agentStore.get().agents.instances[instanceId]?.state;
    if (liveState?.isWorking) {
      throw new Error('Cannot fork a task while it is running');
    }
    if (live) await this.persistAgentState(instanceId);

    const forkId = randomUUID();
    try {
      await this.persistenceDb.forkAgentInstance(
        instanceId,
        forkId,
        throughMessageId,
      );
      await this.attachments.copyAgentBlobs(instanceId, forkId);
      await this.resumeAgent(forkId);
      return forkId;
    } catch (error) {
      await this.persistenceDb.deleteAgentInstance(forkId).catch(() => {});
      await this.attachments.deleteAgentBlobs(forkId).catch(() => {});
      throw error;
    }
  }
  ...
  private async archiveAgent(instanceId: string) {
    ...
    // Persist the final idle state before tearing down the live instance.
    // Archiving preserves the task for later restore, so unlike permanent
    // deletion it must not lose the last input/history changes.
    await this.persistAgentState(instanceId);
    await agent.stop();
  ...
  private async getAgentsHistoryList(
    ...
    return await this.chatPersistence.getSessions(
      offset,
      limit,
      searchString,
      archived,
  ...
  private async getChatProjects(
    offset: number,
    limit: number,
    searchString?: string,
    archived = false,
  ): Promise<ChatProject[]> {
    return await this.chatPersistence.getProjects(
      offset,
      limit,
      searchString,
      archived,
