...
describe('state-mutations/streaming', () => {
  ...
  it('persists the recovered cloud sequence on the existing assistant message', () => {
    ...
    upsertAgentInstance(store, 'a1', makeEnvelope(baseState([assistant])));

    markRecoveredCloudSequence(store, 'a1', {
      messageId: 'asst-1',
      executionId: 'execution-1',
      sequence: 6,
      recoveredAt: '2026-07-11T10:00:00.000Z',
    });

    const persisted = store.get().agents.instances.a1!.state.history[0]!;
    expect(persisted.parts).toEqual(assistant.parts);
    expect(persisted.metadata?.cloudReplay).toEqual({
      executionId: 'execution-1',
      lastSequence: 6,
      recoveredAt: '2026-07-11T10:00:00.000Z',
    });
