...
const ALLOWLIST = new Set([
  ...
  // Pending-edit service: owns the `toolbox.pendingProposedEdits` preview
  // lifecycle for write/multiEdit approval. Adds/removes previews with one
  // `store.update()` per explicit user-review intent.
  'packages/agent-core/src/services/pending-edits/index.ts',
  ...
  // Browser Agent OS owns a separate persisted `AgentOsStateStore`, not the
  // core `AgentStore`. Its feature services are the intentional writers for
  // their respective `agentOs` slices and sync through that store's single
  // persistence/Karton controller.
  'apps/browser/src/backend/services/agent-os/browser-use-policy.ts',
  'apps/browser/src/backend/services/agent-os/chronicle.ts',
  'apps/browser/src/backend/services/agent-os/debug-inspector.ts',
  // Desktop automation owns the `desktopAutomation` slice in the separate
  // AgentOsStateStore; it does not mutate the core AgentStore.
  'apps/browser/src/backend/services/agent-os/desktop-automation.ts',
  'apps/browser/src/backend/services/agent-os/guardian-feedback.ts',
  'apps/browser/src/backend/services/agent-os/hooks.ts',
  'apps/browser/src/backend/services/agent-os/index.ts',
  'apps/browser/src/backend/services/agent-os/micro-controller.ts',
  'apps/browser/src/backend/services/agent-os/remote-control.ts',
  'apps/browser/src/backend/services/agent-os/skill-installer.ts',
]);
