...
import { AeadDataProtection, isDataProtectionEnvelopeBuffer } from '../../host';
...
describe('ProcessedImageCacheService – protected persistence', () => {
  it('stores encrypted image bytes and decrypts cache hits', async () => {
    const url = await freshDbUrl();
    const protection = new AeadDataProtection(randomBytes(32));
    const raw = await makePng(32, 32);
    const processed = await makeWebp(16, 16);
    const svc = await ProcessedImageCacheService.createWithUrl(
      url,
      noopLogger,
      protection,
    ...
    expect((await svc.get(raw, CONSTRAINT_OPENAI))?.buf).toEqual(processed);
    await svc.teardown();

    const client = createClient({ url });
    try {
      const result = await client.execute(
        'SELECT result_data FROM processed_image_cache LIMIT 1',
      );
      const stored = Buffer.from(result.rows[0]?.result_data as Uint8Array);
      expect(stored.includes(processed)).toBe(false);
      expect(isDataProtectionEnvelopeBuffer(stored)).toBe(true);
    } finally {
      client.close();
    }
  });

  it('migrates legacy plaintext image rows and rejects a wrong key', async () => {
    const url = await freshDbUrl();
    const raw = await makePng(32, 32);
    const processed = await makeWebp(16, 16);
    const legacy = await ProcessedImageCacheService.createWithUrl(
      url,
      noopLogger,
    );
    await legacy.set(raw, CONSTRAINT_OPENAI, {
      buf: processed,
      mediaType: 'image/webp',
    });
    ...
    const protection = new AeadDataProtection(randomBytes(32));
    const migrated = await ProcessedImageCacheService.createWithUrl(
      url,
      noopLogger,
      protection,
    );
    expect((await migrated.get(raw, CONSTRAINT_OPENAI))?.buf).toEqual(
      processed,
    );
    await migrated.teardown();

    await expect(
      ProcessedImageCacheService.createWithUrl(
        url,
        noopLogger,
