...
export class DiffHistoryService extends DisposableService {
  ...
  private readonly protectedFiles: AgentHost['protectedFiles'];
  ...
  private constructor(deps: DiffHistoryServiceDeps) {
    ...
    this.protectedFiles = deps.host.protectedFiles;
  ...
  public async acceptPendingEditsForAgentFile(
    agentInstanceId: string,
    filePath: string,
  ): Promise<void> {
    const pendingDiffs =
    ...
    const hunkIds = pendingDiffs
      ...
      .flatMap((diff) =>
        !diff.isExternal ? diff.hunks.map((h) => h.id) : [diff.hunkId],
      );
    if (hunkIds.length === 0) return;
    await this.acceptAndRejectHunks(hunkIds, []);
  }
