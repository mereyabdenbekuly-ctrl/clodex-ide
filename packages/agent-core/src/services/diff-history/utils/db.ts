...
import { protectedFileContext, type ProtectedFileStorage } from '../../../host';
...
export async function storeLargeContent(
  ...
  if (protectedFiles) {
    const hashingSource = async function* (): AsyncGenerator<Buffer> {
  ...
  if (protectedFiles) {
    await protectedFiles.commitPreparedFile(tempPath, blobPath);
  } else {
...
export async function* streamContent(
  ...
  assertContentOid(oid);
  const blobPath = path.join(blobsDir, oid);
  let stream: AsyncIterable<Uint8Array>;
  if (protectedFiles) {
    if (!(await protectedFiles.isProtectedFile(blobPath))) {
      throw new Error(
        'Diff-history blob is plaintext while protected storage is enabled',
      );
    }
    stream = protectedFiles.readChunks(
      blobPath,
      protectedFileContext.diffHistoryBlobStore(),
    );
  } else {
  ...
  if (hash.digest('hex') !== oid) {
    throw new Error('Diff-history blob content hash does not match OID');
  }
}

function assertContentOid(oid: string): void {
  if (!/^[a-f0-9]{64}$/.test(oid)) {
    throw new Error('Diff-history blob OID is invalid');
  }
}
...
export async function copyContentToPath(
  ...
  const readStream = Readable.from(
    streamContent(blobsDir, oid, protectedFiles),
  );
