...
describe('diff-history db utilities', () => {
  ...
  describe('LFS (Large File Storage)', () => {
    ...
    it('protects external blobs at rest and decrypts them only through trusted reads', async () => {
      const plaintext = Buffer.from(
        'private diff-history payload that must not appear on disk',
      );
      ...
      const oid = await storeLargeContent(
        db,
        blobsDir,
        bufferToStream(plaintext),
        '/protected/private.bin',
        {
          operation: 'baseline',
          contributor: 'user',
          reason: 'init',
      ...
      const blobPath = path.join(blobsDir, oid);
      const ciphertext = fs.readFileSync(blobPath);

      expect(await protectedFiles.isProtectedFile(blobPath)).toBe(true);
      expect(ciphertext.includes(plaintext)).toBe(false);

      const streamed: Buffer[] = [];
      for await (const chunk of streamContent(blobsDir, oid, protectedFiles)) {
        streamed.push(chunk);
      }
      expect(Buffer.concat(streamed)).toEqual(plaintext);

      const destPath = path.join(os.tmpdir(), `protected-copy-${Date.now()}`);
      try {
        await copyContentToPath(blobsDir, oid, destPath, protectedFiles);
        expect(fs.readFileSync(destPath)).toEqual(plaintext);
      } finally {
        fs.rmSync(destPath, { force: true });
      }
    });

    it('fails closed for plaintext, invalid, and content-mismatched protected blobs', async () => {
      const protectedFiles = new ProtectedFileStorage(
        new AeadDataProtection(randomBytes(32)),
        { chunkSize: 4096 },
      );
      const plaintext = Buffer.from('expected protected blob');
      const oid = computeOid(plaintext);
      await fs.promises.mkdir(blobsDir, { recursive: true });
      fs.writeFileSync(path.join(blobsDir, oid), plaintext);

      await expect(async () => {
        for await (const _chunk of streamContent(
          blobsDir,
          oid,
          protectedFiles,
        )) {
      ...
      }).rejects.toThrow('OID is invalid');

      const mismatchedOid = computeOid(Buffer.from('different payload'));
      await protectedFiles.writeFile(
        path.join(blobsDir, mismatchedOid),
        plaintext,
        'diff-history/blobs/content-addressed',
      );
      await expect(async () => {
        for await (const _chunk of streamContent(
          blobsDir,
      ...
      }).rejects.toThrow('does not match OID');
    });
