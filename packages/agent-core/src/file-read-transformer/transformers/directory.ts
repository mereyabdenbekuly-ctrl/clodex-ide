  ...
  try {
    ...
    if (preview) {
      ...
      const entries = await buildTreeEntries(
        ...
        0,
        previewDepth,
        mountedPath === 'shells' || mountedPath.startsWith('shells/'),
      );
...
async function buildTreeEntries(
  ...
  for (const dirent of dirents) {
    if (hideProtectedSegments && dirent.name.endsWith('.segments')) continue;
