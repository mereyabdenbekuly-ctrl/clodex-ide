...
import { appendFile, mkdir, rename, rm, stat, writeFile } from '../../../fs';
import type { AgentHost } from '../../../host/host';
import { protectedFileContext, readPossiblyProtectedFile } from '../../../host';
...
export class AgentMemoryWriter {
  ...
  private async restoreArchiveState(
    ...
    try {
      const raw = await this.readMemoryText(this.metadataPath);
  ...
  private async hasExpectedByteState(
    ...
    try {
      if (this.host.protectedFiles) {
        const [markdown, jsonl] = await Promise.all([
          this.readMemoryBuffer(this.historyMarkdownPath),
          this.readMemoryBuffer(this.historyJsonlPath),
        ]);
        return (
          markdown.byteLength === archive.mdBytes &&
          jsonl.byteLength === archive.jsonlBytes
        );
      }
  ...
  private async fullRewrite(
    ...
    await Promise.all([
      this.writeMemoryFile(this.historyMarkdownPath, history.markdown),
      this.writeMemoryFile(this.historyJsonlPath, history.jsonl),
  ...
  private async appendNewMessages(
    ...
    if (this.host.protectedFiles) {
      // Protected files are immutable committed objects. Rewriting here keeps
      // every update atomic and avoids exposing an appendable ciphertext file.
      return this.fullRewrite(options);
    }
  ...
  private async computeArchiveState(
    ...
    const [mdBytes, jsonlBytes] = this.host.protectedFiles
      ? await Promise.all([
          this.readMemoryBuffer(this.historyMarkdownPath).then(
            (value) => value.byteLength,
          ),
          this.readMemoryBuffer(this.historyJsonlPath).then(
            (value) => value.byteLength,
          ),
        ])
      : await Promise.all([
          stat(this.historyMarkdownPath).then((value) => value.size),
          stat(this.historyJsonlPath).then((value) => value.size),
        ]);
    return {
  ...
  private async writeMetadataAndIndex(
    ...
    await upsertMemoryIndexEntry(
      ...
      (filePath, content) => this.writeMemoryFile(filePath, content),
      (filePath) => this.readMemoryText(filePath),
    );
  }

  private memoryContext(filePath: string): string {
    ...
    if (
      ...
      throw new Error('Memory file path is outside memory storage');
    }
    return protectedFileContext.memory(relative);
  }

  private async readMemoryBuffer(filePath: string): Promise<Buffer> {
    return readPossiblyProtectedFile(
      this.host.protectedFiles,
      filePath,
      this.memoryContext(filePath),
    );
  }

  private async readMemoryText(filePath: string): Promise<string> {
    return (await this.readMemoryBuffer(filePath)).toString('utf-8');
  }
  ...
  private async writeMemoryFile(
    ...
    if (this.host.protectedFiles) {
      await this.host.protectedFiles.writeFile(
        filePath,
        content,
        this.memoryContext(filePath),
      );
