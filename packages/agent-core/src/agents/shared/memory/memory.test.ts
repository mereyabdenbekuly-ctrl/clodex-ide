...
import { AeadDataProtection, protectedFileContext } from '../../../host';
...
function makeProtectedHost(memoryDir: string): AgentHost {
  const host = makeHost(memoryDir);
  return createTestAgentHost({
    paths: host.paths,
    logger: host.logger,
...
describe('agent memory writer', () => {
  it('stores all memory archive and index files as protected ciphertext', async () => {
    const root = await mkdtemp(path.join(tmpdir(), 'agent-memory-protected-'));
    try {
      const host = makeProtectedHost(root);
      await writeAgentMemorySnapshot({
        host,
        agentInstanceId: 'agent-1',
        title: 'Private Agent Title',
        activeModelId: 'model-a',
        history: [makeMessage('u1', 'user', 'private memory body')],
        reason: 'user-message',
      });

      const files = [
        'agents/agent-1/history.md',
        'agents/agent-1/history.jsonl',
        'agents/agent-1/metadata.json',
        'index.md',
        'index.json',
      ];
      for (const relative of files) {
        ...
        expect(
          (await readFile(filePath)).subarray(0, 8).toString('ascii'),
        ).toBe('CLODEXPF');
        const plaintext = await host.protectedFiles!.readText(
          filePath,
          protectedFileContext.memory(relative),
        );
        expect(plaintext.length).toBeGreaterThan(0);
      }
      expect(
        await readFile(path.join(root, 'index.md'), 'utf-8'),
      ).not.toContain('Private Agent Title');
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  });
