...
async function rebuildRegistryFromMetadata(
  ...
  const metadatas = await readMemoryMetadatas(memoryDir, readText);
...
async function readRegistry(
  ...
  try {
    const raw = await readText(path.join(memoryDir, 'index.json'));
    ...
    if (
      ...
      for (const entry of Object.values(parsed.agents)) {
        if (!isIndexEntry(entry)) {
          return rebuildRegistryFromMetadata(memoryDir, readText);
        }
...
async function upsertMemoryIndexEntryLocked(
  ...
  const registry = await readRegistry(memoryDir, readText);
...
export function upsertMemoryIndexEntry(
  ...
  entry: AgentMemoryIndexEntry,
  writeFileAtomic: (filePath: string, content: string) => Promise<void>,
  readText: (filePath: string) => Promise<string> = (filePath) =>
  ...
  const next = previous
    .catch(() => {})
    .then(() =>
      upsertMemoryIndexEntryLocked(memoryDir, entry, writeFileAtomic, readText),
    );
