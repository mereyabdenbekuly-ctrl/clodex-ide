 ...
 * captured from the clodex OpenAI-compatible gateway. Replay them only
 * when their signature shape clearly identifies the current clodex
...
function selectReasoningDetailsForSource(
  ...
  const canReplayLegacy =
    reasoningSignatureSource?.providerMode === 'clodex' &&
...
>(
  ...
  const cleanedMessage = {
    ...
    parts: message.parts
      ...
      // clodex/legacy signature metadata makes UI reasoning unsafe.
      //
      ...
      // ... cannot be modified`.
      //
      // When no clodex/legacy signature metadata exists we leave
 ...
 * `cache_control` reaches the clodex gateway without any extra
 * transform logic.
 */
