...
const serializeToolPart = (
  ...
  switch (part.type) {
    ...
    case 'tool-getFileSkeleton': {
      const path = escapeSummaryValue(input.path);
      if (err) return `[skeleton: ${path}${err}]`;
      return `[skeleton: ${path}]`;
    }

    case 'tool-getSymbolBody': {
      const path = escapeSummaryValue(input.path);
      const symbolName = escapeSummaryValue(input.symbolName);
      if (err) return `[symbol: ${symbolName} in ${path}${err}]`;
      return `[symbol: ${symbolName} in ${path}]`;
    }

    case 'tool-searchProjectSymbols': {
      const query = escapeSummaryValue(input.query);
      if (err) return `[project symbols: "${query}"${err}]`;
      return `[project symbols: "${query}"]`;
    }
