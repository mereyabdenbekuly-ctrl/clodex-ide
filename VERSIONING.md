...
This document describes the versioning strategy for the Clodex Agentic IDE monorepo.
...
- `clodex` - The desktop browser app (apps/browser, product name is Clodex Agentic IDE)
- `karton` - The karton package (packages/karton)
- `website` - The website (apps/website)
- `stage-ui` - Stage UI components (packages/stage-ui)

**Important:** Sub-scopes like `clodex-ui` are NOT valid. Use the parent package scope. Use `clodex` (not `browser`) for changes to the Electron app — the folder is still `apps/browser`, while the package, tag prefix, and commit scope are all `clodex`.

### Examples

```bash
# Good
feat(clodex): add dark mode toggle
fix(karton): resolve connection timeout issue
docs(clodex): update installation guide

# Bad - will be rejected
feat(clodex-ui): add button component  # Sub-scope not allowed
...
```
feat(clodex)!: redesign navigation
...
pnpm version:clodex:alpha
pnpm version:clodex:beta
pnpm version:clodex:release

# Dry run - preview without making changes
pnpm tsx scripts/release/index.ts --package clodex --dry-run

# Abandon current prerelease and start new version cycle
pnpm tsx scripts/release/index.ts --package clodex --channel alpha --new-cycle
```
...
Nightly releases are separate from the release PR flow. `.github/workflows/nightly-release.yml` can be triggered manually and computes a dynamic nightly version without modifying package.json. Alpha and beta are legacy prerelease channels and should not normally be used for feature delivery. A temporary alpha or beta is still required when a promotion gate explicitly needs evidence from `app_release_channel=prerelease`, such as the isolated agent runtime stable-promotion observation window.
...
6. Build artifacts (cross-platform for Clodex Agentic IDE)
7. Create GitHub Release (Clodex Agentic IDE) or publish to npm (karton)
...
3. Enter the tag name (e.g., `clodex@1.0.0-alpha001`)
...
git tag -d clodex@1.0.0-alpha001
git push origin :refs/tags/clodex@1.0.0-alpha001
...
- **Git tags:** `@clodex/karton@<version>`
...
3. Create initial git tag:
   ...
   ```bash
   git tag @clodex/new-package@0.0.1
   git push --tags
   ```
...
1. Create a markdown file in `.release-notes/`:
   ...
   ```
   .release-notes/clodex.md   # For browser releases
...
1. Your commits use the correct scope: `feat(clodex): ...`
2. There are actual changes to release
3. Sub-scopes like `clodex-ui` need to be changed to `clodex`
...
pnpm version:clodex:alpha --new-cycle
