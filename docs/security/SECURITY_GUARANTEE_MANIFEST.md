...
| INV-OS-ADAPTER-001    | Fixed filesystem/Git/test capabilities are confined by descriptor-relative resolution and a digest-pinned OS sandbox | Linux Node adapters | IMPLEMENTED_UNVERIFIED | `@clodex/adapters-node` adds a pinned ELF/openat2 helper, fixed create, fail-closed replace/mkdir inspection with both execution paths disabled pending pinned private same-filesystem staging, deterministic tree commitments, digest-pinned read-only/networkless Git and test containers, limits, seccomp and AppArmor inputs | The Linux x64 native harness is CI-wired, but exact production filesystem/architecture coverage, container/LSM enforcement, daemon-socket protection, deployment profile enforcement, and production wiring are not yet proven |
