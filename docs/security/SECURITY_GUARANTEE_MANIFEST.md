...
| INV-OS-ADAPTER-001    | Fixed filesystem/Git/test capabilities are confined by descriptor-relative resolution and a digest-pinned OS sandbox | Linux Node adapters | IMPLEMENTED_UNVERIFIED | `@clodex/adapters-node` adds a pinned ELF/openat2 helper, fixed create/replace, fail-closed mkdir inspection with execution disabled pending private same-filesystem staging, deterministic tree commitments, digest-pinned read-only/networkless Git and test containers, limits, seccomp and AppArmor inputs | Native helper/container/LSM code was not compiled or tested; strict namespace CAS, daemon-socket protection, deployment profile enforcement, and production wiring are not yet proven |
