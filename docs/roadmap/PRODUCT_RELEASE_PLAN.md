...
| Provenance/license boundary | The reopened OCB-006 source gaps are remediated: the automated gate reports `835 components; 0 blockers; Nucleo=NOT_REQUIRED`, with immutable source/digest/license evidence for bundled `vscode-eslint`, its nine embedded production dependencies, and the Windows VC++ runtime archive/DLLs | Rerun inspection on the exact final macOS/Windows/Linux artifacts, retain the attested CycloneDX SBOM/publication receipt, and obtain release-owner/specialist review for conditional Microsoft runtime and custom/LGPL obligations |
