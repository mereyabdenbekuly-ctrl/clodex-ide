...
The ten independent Session 5 `@clodex/*` packages now cover contracts,
Guardian, kernel, runtime, canonical approval, logical ledger, signed evidence,
capability-scoped reference adapters, a trusted-local-POSIX ledger adapter, and
fail-closed promotion assessment. They do not change any domain above from
`Legacy`, do not execute against the host workspace, and are not a production
shadow authority. The POSIX adapter's isolated durability claim does not make
kernel/effect/evidence state one atomic production transaction.
